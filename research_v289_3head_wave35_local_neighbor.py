...
OUT=Path('analysis_v289_3head_wave35_local_neighbor.csv')
OUTJ=Path('research_v289_3head_wave35_local_neighbor.json')
OUTM=Path('research_v289_3head_wave35_local_neighbor.md')
...
def fit_predict(trainX,train_combo,testX):
    ...
    k=min(N_NEIGHBORS,len(A))
    nn=NearestNeighbors(n_neighbors=k,metric='euclidean',n_jobs=-1).fit(A)
    dist,idx=nn.kneighbors(B,return_distance=True)
    w=1.0/(dist+0.25)
    head_score=(w*y[idx]).sum(axis=1)/w.sum(axis=1)
    mask=y==1
...
def main():
    ...
    if len(exclusion)!=94 or bdf['race_code'].nunique()!=94: raise RuntimeError('baseline exclusion must be exact unique 94')
    ...
    out={'wave':'35-local-neighbor','family':'distance-weighted nearest-neighbor local P(3-head) + conditional logistic order','feature_count':63,'neighbors':N_NEIGHBORS,'march_tuning_from_feb_only':chosen,'strict_pristine_holdout_apr_jun':hm,'shadow_non_pristine_jul_aug':sm,'legacy_overlap':overlap,'combined_baseline_plus_holdout':cmb,'decision':decision,'scope_note':'Wave20 full six-boat population minus exact v288 operational 94R only.'}
    OUTJ.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    lines=['# Wave35 local-neighbor corrected scope','',f"- March gate: **score>={cut:.6f}, top{k}; {chosen['races']}R / ROI {chosen['roi_pct']:.3f}%**",f"- Apr-Jun holdout: **{hm['races']}R / {hm['hits']} hits / ROI {hm['roi_pct']:.3f}% / profit {hm['profit_yen']:+,} yen**",f"- min month: **{hm['min_month_roi_pct']:.3f}%** / red months **{hm['red_months']}** / max DD **{hm['max_drawdown_yen']:,.0f} yen**",f"- Jul-Aug shadow: **{sm['races']}R / {sm['hits']} hits / ROI {sm['roi_pct']:.3f}% / profit {sm['profit_yen']:+,} yen**",f"- baseline + holdout: **{cmb['races']}R / ROI {cmb['roi_pct']:.3f}% / profit {cmb['profit_yen']:+,} yen**",f'- overlap: **{overlap}**',f'- decision: **{decision}**','','## Holdout monthly']
