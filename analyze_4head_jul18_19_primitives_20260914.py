...
from backtest_v4 import resistance12,ingest_prior_day_preview
from backtest_v5_ev import process_features
from backtest_v18_core45 import features4
import pandas as pd,json
DATES=['2026/07/18','2026/07/19']
# Build causal v4/v5 feature state so legacy_score4 is taken from the same replay path.
cache={};hist=defaultdict(list);seen=set();d=date(2026,3,1);targets={}
while d<=date(2026,7,19):
 feats=process_features(d,cache,hist)
 if str(d).replace('-','/') in DATES:
  targets[str(d).replace('-','/')]={r['レースコード']:(x,s4) for r,x,s4,_s5,_dc in feats}
...
for ymd in DATES:
 ...
 for r in c:
  code=r.get('レースコード');q=wm.get(code,{});x=race_features(r,q);f=features4(x);full=targets.get(ymd,{}).get(code);s4=full[1] if full else float('nan')
  z={'date':ymd,'resistance12':resistance12(x),'wall3_weak':f['3壁弱さ'],'past_win4':x[4]['past_win'],'legacy_score4':s4}
  for b in range(1,5):
...
J={'outcome_blind':True,'production_modified':False,'causal_feature_replay':True,'days':audit,'schema_diff':diff};open('audit_4head_jul18_19_primitives_20260914.json','w').write(json.dumps(J,ensure_ascii=False,indent=2))
