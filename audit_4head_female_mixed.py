...
def main():
 ...
 def sexval(r,b):
  for k in (f'艇{b}_性別',f'{b}号艇_性別',f'艇{b}_選手性別'):
   v=str(r.get(k,'')).strip()
   if v:return v
  return ''
 def female(v): return v in ('女','女子','F','2')
 def male(v): return v in ('男','男子','M','1')
 def enrich(q):
  q=q.copy(); vals=[]
  ...
  q['target_female_mixed']=(q.boat4_sex.map(female)&q.male_count.ge(2)&q.sex_known_count.eq(6))
  return q
 out={}
 details=[]
