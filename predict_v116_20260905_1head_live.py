...
This is NOT a new model. It freezes the v109 architecture/cuts and refits only on the
already-settled v108 table through 2026-08-31. For 2026-09-05, no result/payout/odds
files are read. Final A/S is emitted only when current TKZ + STT + original exhibition
are all present and boat 1 remains course 1. Future/incomplete races are shown only as
neutral-preview provisional candidates and MUST be re-scored after exhibition.
"""
...
OUT='prediction_v116_20260905_1head_live.csv'
SUMMARY='summary_v116_20260905_1head_live.md'
A_CUT=.65
S_CUT=.72
VENUE_NAMES={
...
def fetch_st_day(d):
    y=d.strftime('%Y/%m/%d')
    return d,rows(f'data/previews/stt/{y}.csv')

def exact_prior_st_bias():
    # Same v108 rule: strictly earlier dates only. Extend through 2026-09-04 for Sep 5.
    ds=[];d=date(2025,10,1)
...
def deadline_from_preview(code,*maps):
    for m in maps:
        r=m.get(code,{})
        x=(r.get('締切時刻') or '').strip()
        if x:return x
    return ''
...
def main():
    ...
    for card in cards:
        ...
        rno=ii(card.get('レース回'))
        name=(card.get('艇1_選手名') or '').strip()
        g1=(card.get('艇1_級別') or '').strip()
        complete=code in tkz and code in stt and code in orig
        dl=deadline_from_preview(code,tkz,stt,orig)
        # neutral-preview p only to prioritize races whose current exhibition has not been published.
        provisional=feature_row(HD.isoformat(),card,waku[code],{},{},{},bias)
        ppre=pred(model,provisional) if provisional is not None else float('nan')
        ppre_grade=grade(ppre) if np.isfinite(ppre) else '-'
        status='PENDING_EXHIBITION';plive='';glive='';entry='missing'
        if complete:
            sr=stt[code];entry='same' if ii(sr.get('艇1_コース'))==1 else 'changed'
            live=feature_row(HD.isoformat(),card,waku[code],tkz,stt,orig,bias)
            if live is None:
                status='ENTRY_CHANGED_EXCLUDE'
            else:
                p=pred(model,live);plive=f'{p:.8f}';glive=grade(p);status='LIVE_FINAL'
        past=''
        if dl:
            try:
                t=datetime.strptime(HD.isoformat()+' '+dl,'%Y-%m-%d %H:%M').replace(tzinfo=ZoneInfo('Asia/Tokyo'))
                past='1' if t<=now else '0'
            except Exception:past=''
        out.append({'race_code':code,'venue':venue,'venue_name':VENUE_NAMES.get(venue,venue),'race':rno,
                    'deadline':dl,'boat1_name':name,'boat1_grade':g1,'status':status,'entry_status':entry,
                    'preview_complete':int(complete),'p109_live':plive,'grade_live':glive,
                    'p109_neutral_pre':f'{ppre:.8f}' if np.isfinite(ppre) else '',
                    'grade_neutral_pre':ppre_grade,'closed_at_scan':past})
    # Sort known deadline first; blank deadline later.
    ...
    L=['# v116 2026-09-05 1号艇モデル live scan','',
       ...
       f'- today cards: **{len(out)}R**',
       f'- live final A/S targets at scan: **{len(live_targets)}R**',
       f'- pending neutral-pre A/S priority: **{len(pending_pre)}R**',
       f'- entry changed exclusions: **{len(changed)}R**','',
       '## 現時点で正式A/S','|締切|場|R|1号艇|級|p109|判定|締切済|','|---|---|---:|---|---|---:|---|---|']
    for r in live_targets:
        L.append(f"|{r['deadline']}|{r['venue_name']}|{r['race']}R|{r['boat1_name']}|{r['boat1_grade']}|{100*float(r['p109_live']):.1f}%|{r['grade_live']}|{'済' if r['closed_at_scan']=='1' else '未'}|")
    if not live_targets:L.append('|-|-|-|-|-|-|-|-|')
    L+=['','## 今後の展示待ち・neutral-pre A/S（正式判定ではない）','|場|R|1号艇|級|neutral p|仮層|','|---|---:|---|---|---:|---|']
    for r in pending_pre:
        L.append(f"|{r['venue_name']}|{r['race']}R|{r['boat1_name']}|{r['boat1_grade']}|{100*float(r['p109_neutral_pre']):.1f}%|{r['grade_neutral_pre']}|")
    if not pending_pre:L.append('|-|-|-|-|-|-|')
    if changed:
        L+=['','## 1コース変更で除外']
        for r in changed:L.append(f"- {r['venue_name']}{r['race']}R ①{r['boat1_name']}")
