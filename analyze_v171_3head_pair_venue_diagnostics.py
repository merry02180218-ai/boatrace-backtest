...
Regenerates frozen v166 Jun-Aug rows from v165/v166 logic, then performs diagnostics.
No retuning, no venue filtering, payout settlement-only.
"""
import csv
from collections import Counter
from analyze_v166_3head_pair_direct import read,ff,ii,combo,choose,score_month,SRC,HEADSRC,TEST_MONTHS
OUT='analysis_v171_3head_pair_venue_diagnostics.csv'; SUMMARY='summary_v171_3head_pair_venue_diagnostics.md'
CUT=.30; N=10
...
def main():
    allrows,lam=build_v166_rows()
    a=[r for r in allrows if ff(r.get('p3head'))>=CUT]
    rows=[]
    for v in sorted(set(vc(r) for r in a)):
        ...
        ret=sum(ff(r.get('payout100')) for r in settled if 0<ii(r.get('v166_rank20'))<=N); inv=len(settled)*N*100
    ...
    L=['# v171 3号艇 2着3着・場別診断','',f'- Frozen: p3 >= {CUT:.2f}, v166 top {N}, λ={lam:.2f}.','- v166 Jun-Aug rows are regenerated from the frozen v165/v166 logic at runtime.','- ③頭になったレースだけで、2着艇を候補内に含めた率・3着艇を含めた率・ordered pairまで当たった率を場別比較。','- Jun-Aug結果を使った再学習・場除外はしない。','','|venue|候補R|③頭R|③頭率|2着leg|3着leg|両leg|pair coverage|ROI|','|---|---:|---:|---:|---:|---:|---:|---:|---:|']
