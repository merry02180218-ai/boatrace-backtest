...
TRAIN=('2026-04','2026-05','2026-06');HOLD=('2026-07','2026-08');PLAYER_CUTS=(0.210863,0.215605)
...
def build_ex(wanted):
 ...
 while d<=END:
  ...
  # March is recorded for the dedicated Mar-Aug audit. Apr-Aug values are unchanged.
  if d>=date(2026,3,1):
   ...
   for c in [x for x in wanted if x.startswith(d.strftime('%Y%m%d'))]:
    tr,sr,orr=tkz.get(c,{}),stt.get(c,{}),orig.get(c,{});audit=v326.raw_completeness(tr,sr,orr);basic=int(bool(audit['tkz_all6']) and bool(audit['stt_all6']));turn_av=int(bool(audit['orig_turn_all6']));straight_av=int(bool(audit['orig_straight_all6']));avg_av=int(bool(audit['orig_avg_all6']))
    ...
    if basic:
     ex,st,os=corrected_direct(c,tkz,stt,orig,bias);inside=(1,2,3);others=(1,2,3,5,6);z.update({'ex4_adv_all':avg([ex[b] for b in others])-ex[4],'ex4_adv_inside':avg([ex[b] for b in inside])-ex[4],'st4_adv_all':avg([st[b] for b in others])-st[4],'st4_adv_inside':avg([st[b] for b in inside])-st[4]})
     if turn_av:
...
def main():
 z=base.settle_all().merge(base.build_motor_features(),on=['race_code','month'],how='inner');z=z[(z.motor_win_diff_4v3>=base.WIN_CUT)&(z.motor_2ren_diff_4v3>=base.REN2_CUT)];z=z.merge(base.build_prior_features(),on=['race_code','month'],how='left');z['race_code']=z.race_code.astype(str).str.zfill(12);z['head4']=pd.to_numeric(z.actual_head4,errors='coerce').fillna(0).astype(int);ex=build_ex(set(z.race_code));z=z.merge(ex,on='race_code',how='left');z.to_csv(OUT/'detail.csv',index=False);print('Availability audit ready; September UNREAD')
