...
BASIC_FEATS=['ex4_adv_all','ex4_adv_inside','st4_adv_all','st4_adv_inside']
ORIG_FEATS=['turn4_adv_inside','straight4_adv_inside','orig4_adv_inside','straight4_minus3','turn4_minus3']
 ...
 while d<=END:
  ...
  if d>=date(2026,4,1):
   ...
   for c in [x for x in wanted if x.startswith(d.strftime('%Y%m%d'))]:
    ...
    orig_complete=int(all(bool(audit[k]) for k in ('orig_turn_all6','orig_straight_all6','orig_avg_all6')))
    z={'race_code':c,'venue_code':c[8:10],'basic_complete':basic,'orig_complete':orig_complete,'orig_row_present':int(bool(orr)),**audit}
    if basic:
     # corrected_direct can accept an empty original row; basic exhibition/ST are independent of orig availability.
...
def main():
 ...
 venue=(z.groupby('venue_code',dropna=False).agg(R=('race_code','size'),basic_complete=('basic_complete','sum'),orig_complete=('orig_complete','sum'),orig_rows=('orig_row_present','sum')).reset_index())
 venue.to_csv(OUT/'venue_availability.csv',index=False)
 print('VENUE_AVAILABILITY');print(venue.to_string(index=False))
 grid=[]
 for pc in PLAYER_CUTS:
  b=z[z.player4_all_win>=pc];tr=b[b.month.isin(TRAIN)];ho=b[b.month.isin(HOLD)]
  print('BASE',pc,metric(tr),'HOLD',metric(ho),'basic',int(tr.basic_complete.fillna(0).sum()),'orig',int(tr.orig_complete.fillna(0).sum()),'/',len(tr))
  for f in BASIC_FEATS+ORIG_FEATS:
   completeness='basic_complete' if f in BASIC_FEATS else 'orig_complete'
   tt=tr[(tr[completeness]==1)&tr[f].notna()];hh=ho[(ho[completeness]==1)&ho[f].notna()]
   if len(tt)<25:continue
   for q in np.arange(.15,.86,.05):
    cut=float(tt[f].quantile(q));a=tt[tt[f]>=cut]
    if len(a)<25:continue
    grid.append({'player_cut':pc,'feature':f,'availability':completeness,'q':q,'cut':cut,**{f'train_{k}':v for k,v in metric(a).items()},**{f'hold_{k}':v for k,v in metric(hh[hh[f]>=cut]).items()}})
 ...
 print('SELECTED_TRAIN_ONLY',best[['player_cut','feature','availability','q','cut','train_R','train_rate']].to_dict())
 print('FIXED_HOLD',{'R':int(best.hold_R),'rate':float(best.hold_rate)})
 print('TOP10_TRAIN_ONLY');print(cand[['player_cut','feature','availability','cut','train_R','train_rate','hold_R','hold_rate']].head(10).to_string(index=False))
