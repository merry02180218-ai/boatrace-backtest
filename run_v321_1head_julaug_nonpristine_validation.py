...
def prepare():
    ...
    d,th=v298.add_threat(d)
    fam=v298.v296.clean_manifest(d)
    d,tf=v303.add_transfer_features(d)
    d,ca=v307.add_causal(d)
    ...
    if any('meet_' in str(c).lower() for c in safe):
        raise RuntimeError('v321 unsafe head feature')
    ...
    if any(str(m).startswith('2026-09') for m in d.month.astype(str).unique()):
        raise RuntimeError('v321 September present')
    ...
    # Opponent mass is NOT computed here. v303.opponent_mass() is hard-wired to the old
    # Feb-Jun TEST_MONTHS and was both incorrect for Jul/Aug and the OOM-heavy path.
    # We persist only the symmetric opponent inputs, then compute Jul/Aug BASE mass in
    # evaluate() with month M trained strictly on < M.
    v300.setup_v298()
    sufs=v298.suffixes(d)
    if any('meet_' in str(x).lower() for x in sufs):
        raise RuntimeError('v321 forbidden meet suffix')
    ...
    if any(str(m).startswith('2026-09') for m in slim.month.astype(str).unique()):
        raise RuntimeError('v321 slim contains September')
    ...
    if missing:
        raise RuntimeError(f'v321 required head columns missing n={len(missing)} sample={missing[:8]}')
    hd=d[head_cols].copy()
    del d
    gc.collect()
    ...
    for tm in TARGET_MONTHS:
        ...
        tr=hd.loc[hd.month<tm].copy()
        te=hd.loc[hd.month==tm].copy()
        if te.empty:
            raise RuntimeError(f'v321 no source rows for {tm}')
        bc=v298.v293.available(tr,core,.55)
        bg=v298.v293.available(tr,guard,.55)
        ex=v303.available(tr,safe)
...
def evaluate():
    if not (PREP_SLIM.exists() and PREP_HEAD.exists() and PREP_META.exists()):
        raise RuntimeError('v321 prepare cache missing')
    slim=pd.read_csv(PREP_SLIM,dtype={'race_code':str})
    head=pd.read_csv(PREP_HEAD,dtype={'race_code':str})
    hcut=float(pd.read_csv(PREP_META).hcut.iloc[0])
    if any(str(m).startswith('2026-09') for m in slim.month.astype(str).unique()):
    ...
    # SECOND: compute frozen v308 BASE opponent mass and frozen v317 SECOND from the
    # same causal long table. BASE uses the historical v300 BASE model (L2=10, no aug).
    ...
    del sl,sl_hr
    gc.collect()
    p2={tm:v311.p2_predict_explicit(sx,tm,1.0,None,{'START'})[0] for tm in TARGET_MONTHS}
    del sx
    gc.collect()

    # THIRD: compute frozen v308 BASE opponent mass counterpart and frozen v318 THIRD.
    ...
    # Reconstruct the v308 opponent-mass gate for Jul/Aug correctly. No use of the old
    # v303.opponent_mass(), whose TEST_MONTHS stop at June.
    masses={}
    for tm in TARGET_MONTHS:
        codes=head.loc[head.month.astype(str)==tm,'race_code'].astype(str).str.zfill(12).unique()
        for code in codes:
            if code not in base_p2[tm] or code not in base_pc[tm]:
                continue
            masses[code]=v300.base5(base_p2[tm][code],base_pc[tm][code])[1]
    head['opp_mass']=[masses.get(str(x).zfill(12),np.nan) for x in head.race_code]
    if head.opp_mass.isna().all():
        raise RuntimeError('v321 Jul/Aug opponent mass unavailable')
    ...
    if selected.empty:
        raise RuntimeError('v321 no selected races')
    rows=[]
    fn=v299.STRATEGIES[POLICY]
    for _,r in selected.iterrows():
        tm=str(r.month)
        code=str(r.race_code).zfill(12)
        if code not in p2[tm] or code not in pc[tm]:
            raise RuntimeError(f'v321 frozen opponent prediction missing {tm} {code}')
        probs=v299.pair_prob(p2[tm][code],pc[tm][code],ALPHA)
        top3=fn(p2[tm][code],pc[tm][code],probs)[:3]
        if len(top3)!=3 or len(set(top3))!=3:
            raise RuntimeError(f'invalid ticket set {code}')
    ...
    gm['head_rate']=gm.head_hits/gm.R
    gm['hit_rate']=gm.hits/gm.R
    R=len(z)
    hh=int(z.head_hit.sum())
    hits=int(z.hit.sum())
    z.to_csv(str(P)+'_race.csv',index=False)
    gm.to_csv(str(P)+'_monthly.csv',index=False)

    L=['# v321 Jul/Aug 2026 NON-PRISTINE validation','',
       '- **Reference only; NOT pristine and NOT eligible for promotion.**',
       f'- Frozen v308 head gate: development-only q=.98 cutoff **p_head>={hcut:.8f}**, opponent mass >= {OPP_MASS_CUT:.3f}.',
       ...
       f'- Frozen ticket policy: **{POLICY} alpha={ALPHA:.2f}**, exactly 3 tickets.',
       '- `meet_*` forbidden; missing p3/p4 never future-backfilled; September outcomes untouched.','',
       f'## Result\n- Jul+Aug: R **{R}**, head **{hh}/{R}={100*hh/R:.2f}%**, exact3 **{hits}/{R}={100*hits/R:.2f}%**.','',
       '## Monthly','|month|R|head|head rate|exact3|exact3 rate|','|---|---:|---:|---:|---:|---:|']
    for _,r in gm.iterrows():
        L.append(f'|{r.month}|{int(r.R)}|{int(r.head_hits)}|{100*r.head_rate:.2f}%|{int(r.hits)}|{100*r.hit_rate:.2f}%|')
...
if __name__=='__main__':
    ...
    ap.add_argument('--stage',choices=['prepare','evaluate','all'],default='all')
    a=ap.parse_args()
