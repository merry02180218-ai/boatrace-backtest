...
def build_head_selection():
    ...
    if any(str(m).startswith('2026-09') for m in TARGET_MONTHS): raise RuntimeError('September forbidden')
    ...
    for tm in TARGET_MONTHS:
        ...
        out.append(z)
    return pd.concat(out,ignore_index=True),hcut


def build_opponent_probs(cache):
    d,p3,p4=cache
    d=d.copy(); d.race_code=d.race_code.astype(str).str.zfill(12)
    if any('meet_' in str(c) for c in d.columns): raise RuntimeError('v321 meet_* present in cache')
    have=set(d.month.astype(str).unique())
    missing=[m for m in TARGET_MONTHS if m not in have]
    if missing: raise RuntimeError(f'v321 cache missing target months {missing}')
...
def main():
    head,hcut=build_head_selection()
    d,p2,pc=build_opponent_probs(v312.load_cache())
    ...
    if missing: raise RuntimeError(f'v321 selected races absent from v313 cache n={len(missing)} sample={missing[:5]}')
    ...
    L=['# v321 Jul/Aug 2026 NON-PRISTINE validation','',
       ...
       '- Month M trains only on months < M; `meet_*` forbidden; September outcomes untouched.','',
