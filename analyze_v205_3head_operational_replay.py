...
current exhibition margins from v108, and corrected official closing odds only for settlement.
Prediction/race selection never uses odds/result/payout.
"""
...
def load_odds():
    ...
    for p in sorted(OD.glob('*/*/*.csv')):
        try:
            ...
            xs.append(x)
        except Exception as e: print('skip odds',p,e)
...
def settle(r,o):
    ...
    return dict(hit=int(actual in ts and actual_stake>0),composite_odds=comp,cost=BANK,return_yen=ret,profit=ret-BANK,actual_stake=actual_stake)
...
def main():
    ...
    # v195 rows already satisfy actual production v165 p3head>=.30 and have frozen v166 Top10.
    ...
    L += ['', '## PICK UP races','|split|date|race_code|PRE p|p3head|turn|ST|hit|comp|return|profit|','|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|']
    p=z[z.pickup].sort_values(['month','date','race_code'])
    for _,r in p.iterrows():
        L.append(f"|{r.split}|{r.date}|{r.race_code}|{r.pre_prob_eval:.4f}|{r.p3head:.4f}|{r.turn_margin23:.3f}|{r.st_margin23:.3f}|{int(r.hit)}|{r.composite_odds:.3f}|{r.return_yen:.0f}|{r.profit:.0f}|")
    if p.empty:L.append('|-|-|-|-|-|-|-|-|-|-|-|')
