...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--date',required=True);ap.add_argument('--cards',type=Path,required=True);ap.add_argument('--pre',type=Path,required=True);ap.add_argument('--out',type=Path,required=True);a=ap.parse_args();target=pd.Timestamp(a.date).date()
 ...
 pre=pd.read_csv(a.pre,dtype={'race_code':str});pre.race_code=pre.race_code.astype(str).str.zfill(12);pm=pre.set_index('race_code').to_dict('index');hm=head.set_index('race_code');a.out.mkdir(parents=True,exist_ok=True)
 ...
 for code,m in pm.items():
  ...
  mass=v300.base5(base_p2[code],base_pc[code])[1];probs=v299.pair_prob(p2[code],pc[code],prod.TICKET_ALPHA);top=v299.STRATEGIES['HYBRID'](p2[code],pc[code],probs)[:3]
  obj={'race_code':code,'pre_class':m.get('pre_class'),'legacy_pre_p':float(m.get('legacy_pre_p')),'final_head_p':float(hm.loc[code,'p_head']),'opp_mass':float(mass),'p2':{str(k):float(v) for k,v in p2[code].items()},'pc':{f'{s}-{t}':float(v) for (s,t),v in pc[code].items()},'base_tickets':';'.join(f'1-{s}-{t}' for s,t in top),'production_profile':prod.PROFILE_NAME,'training_cutoff':(target-pd.Timedelta(days=1)).isoformat(),'result_or_payout_used':False,'chronology_guard':True};(a.out/f'{code}.json').write_text(json.dumps(obj,ensure_ascii=False,indent=2));n+=1
 meta={'date':a.date,'candidate_cache_R':n,'training_cutoff':(target-pd.Timedelta(days=1)).isoformat(),'production_profile':prod.PROFILE_NAME,'result_or_payout_used':False,'chronology_guard':True};(a.out/'meta.json').write_text(json.dumps(meta,indent=2));print(json.dumps(meta))
