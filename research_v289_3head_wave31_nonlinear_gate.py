...
OUT=Path('analysis_v289_3head_wave31_nonlinear_gate.csv')
OUTJ=Path('research_v289_3head_wave31_nonlinear_gate.json')
OUTM=Path('research_v289_3head_wave31_nonlinear_gate.md')
...
def cond_pipe(): return make_pipeline(SimpleImputer(strategy='median'),StandardScaler(),LogisticRegression(max_iter=400,C=.25,solver='lbfgs'))
...
def fit_predict(trainX,train_combo,testX):
    imp=SimpleImputer(strategy='median');Xi=imp.fit_transform(trainX);Xt=imp.transform(testX)
    yhead=train_combo.str.startswith('3-').astype(int)
    head=HistGradientBoostingClassifier(max_iter=120,learning_rate=.05,max_leaf_nodes=15,min_samples_leaf=40,l2_regularization=2.0,random_state=31)
    head.fit(Xi,yhead);p3=head.predict_proba(Xt)[:,list(head.classes_).index(1)]
    mask=yhead.eq(1);cond=cond_pipe();cond.fit(trainX.loc[mask],train_combo.loc[mask]);pc=cond.predict_proba(testX)
...
def main():
    ...
    X=build_static(df);yc=df['settle__actual_combo'].astype(str);p3=np.full(len(df),np.nan);cs=np.full((len(df),len(COMBOS)),np.nan);pos={idx:i for i,idx in enumerate(df.index)}
    def run(tr,te):
    ...
    sel_h,hm=score(df[df['month'].isin(['2026-04','2026-05','2026-06'])].copy(),t,k);sel_s,sm=score(df[df['month'].isin(['2026-07','2026-08'])].copy(),t,k)
    ...
    out={'wave':'31-nonlinear-head-gate','family':'HistGradientBoosting 3-head gate + conditional exact-order logistic','feature_count':63,'march_tuning_from_feb_only':chosen,'strict_pristine_holdout_apr_jun':hm,'shadow_non_pristine_jul_aug':sm,'legacy_overlap':0,'combined_baseline_plus_holdout':cmb,'decision':decision}
    OUTJ.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    lines=['# Wave31 nonlinear head gate','',f"- March tune (train Feb only): **p3>={t:.2f}, top{k}; {chosen['races']}R / ROI {chosen['roi_pct']:.3f}%**",f"- Apr-Jun untouched holdout: **{hm['races']}R / {hm['hits']} hits / ROI {hm['roi_pct']:.3f}% / profit {hm['profit_yen']:+,} yen**",f"- min month: **{hm['min_month_roi_pct']:.3f}%** / red months **{hm['red_months']}** / max DD **{hm['max_drawdown_yen']:,.0f} yen**",f"- Jul-Aug NON-PRISTINE shadow: **{sm['races']}R / {sm['hits']} hits / ROI {sm['roi_pct']:.3f}% / profit {sm['profit_yen']:+,} yen**",f"- baseline + holdout: **{cmb['races']}R / ROI {cmb['roi_pct']:.3f}% / profit {cmb['profit_yen']:+,} yen**",'- legacy overlap: **0**',f'- decision: **{decision}**','','## Pristine holdout monthly']
