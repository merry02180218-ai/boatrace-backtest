...
OUTJ=Path('research_v289_3head_wave34h_march_stability.json')
OUTM=Path('research_v289_3head_wave34h_march_stability.md')
OUT=Path('analysis_v289_3head_wave34h_march_stability.csv')
...
def evaluate(df,mode,cut,k):
    ...
    q['variant_return']=[base.dutch_return(r,str(r[f'top{k}']).split(';') if r[f'top{k}'] else []) for _,r in q.iterrows()]
...
def main():
    ...
    ms=np.full(n,np.nan); ag=np.full(n,np.nan); scores=np.full((n,len(base.COMBOS)),np.nan)
    ...
    df['motor_mean_confirm']=df[zcols].mean(axis=1); df['motor_min_confirm']=df[zcols].min(axis=1)
    modes=MOTOR_COLS+['motor_mean_confirm','motor_min_confirm']
    for k in KS:
        vals=[]
        for i in range(n):
            if not np.isfinite(scores[i]).any(): vals.append(''); continue
            idx=np.argsort(-np.nan_to_num(scores[i],nan=-1))[:k]; vals.append(';'.join(base.COMBOS[j] for j in idx))
        df[f'top{k}']=vals
    march=df[df.month=='2026-03'].copy().sort_values(['date','race_code']).reset_index(drop=True)
    split=len(march)//2; early=march.iloc[:split].copy(); late=march.iloc[split:].copy(); cand=[]
    for mode in modes:
        for qv in QS:
            cut=float(march[mode].quantile(qv))
            for k in KS:
                _,full=evaluate(march,mode,cut,k); _,em=evaluate(early,mode,cut,k); _,lm=evaluate(late,mode,cut,k)
                if 30<=full['races']<=250 and em['races']>=10 and lm['races']>=10:
                    full['march_early']=em; full['march_late']=lm; full['worst_half_roi']=min(em['roi_pct'],lm['roi_pct']); cand.append(full)
    if not cand: raise RuntimeError('no stable March candidate')
    chosen=max(cand,key=lambda z:(z['worst_half_roi'],z['roi_pct'],z['races'],-modes.index(z['mode'])))
    ...
    out={'wave':'34h-march-stability','feature_count':63,'march_selection':chosen,'strict_pristine_holdout_apr_jun':hm,'shadow_non_pristine_jul_aug':sm,'legacy_overlap':overlap,'combined_baseline_plus_holdout':cmb,'decision':decision}
    OUTJ.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    e=chosen['march_early']; l=chosen['march_late']
    lines=['# Wave34h March stability selection','',f"- March selected: **{chosen['mode']} >= {chosen['cut']:.4f}, top{chosen['k']}**",f"- March full: {chosen['races']}R / ROI {chosen['roi_pct']:.3f}%",f"- March early: {e['races']}R / ROI {e['roi_pct']:.3f}% ; late: {l['races']}R / ROI {l['roi_pct']:.3f}% ; worst-half {chosen['worst_half_roi']:.3f}%",f"- Apr-Jun: **{hm['races']}R / {hm['hits']} hits / ROI {hm['roi_pct']:.3f}% / profit {hm['profit_yen']:+,} yen**",f"- min month {hm['min_month_roi_pct']:.3f}% / red months {hm['red_months']} / max DD {hm['max_drawdown_yen']:,.0f} yen",f"- Jul-Aug shadow: **{sm['races']}R / {sm['hits']} hits / ROI {sm['roi_pct']:.3f}% / profit {sm['profit_yen']:+,} yen**",f"- baseline + holdout: **{cmb['races']}R / ROI {cmb['roi_pct']:.3f}% / profit {cmb['profit_yen']:+,} yen**",f'- overlap: **{overlap}**',f'- decision: **{decision}**','','## Monthly']
