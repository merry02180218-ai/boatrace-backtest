#!/usr/bin/env python3
"""Lane-partitioned six-boat tracker for result-blind live exhibition SES.

v6 preserves v5 quality thresholds/stride/template fusion, but prevents adjacent
boats from converging onto the same wake/background patch by partitioning the
image into dynamic vertical cells derived from the previous-frame boat centers.
LK/template estimates must remain inside the boat's own cell. Crossing/order or
cell collapse fails closed. No race outcomes are read.
"""
...
def dynamic_cells(centers, order, height, margin=3.0):
    ys=centers[order,1].astype(float)
    if np.any(np.diff(ys)<=8.0): return None
    mids=(ys[:-1]+ys[1:])/2.0
    cells={}
    for k,idx in enumerate(order):
        lo=0.0 if k==0 else mids[k-1]+margin
        hi=float(height-1) if k==len(order)-1 else mids[k]-margin
        if hi-lo<18.0: return None
        cells[int(idx)]=(float(lo),float(hi))
    return cells

def template_match_cell(cur_g,template,pred,ylo,yhi,search_x=68,search_y=20):
    ...
    y1=max(int(np.floor(ylo)),int(round(pcy-th/2-search_y)),0)
    y2=min(int(np.ceil(yhi)),int(round(pcy+th/2+search_y)),cur_g.shape[0])
    roi=cur_g[y1:y2,x1:x2]
    if roi.shape[0]<th or roi.shape[1]<tw: return None,None
    res=cv2.matchTemplate(roi,template,cv2.TM_CCOEFF_NORMED)
    _,score,_,loc=cv2.minMaxLoc(res)
    center=np.array([x1+loc[0]+tw/2,y1+loc[1]+th/2],np.float32)
    if not (ylo <= float(center[1]) <= yhi): return None,None
...
def main():
    ...
    ap.add_argument('--seed-json',type=Path,required=True); ap.add_argument('--out',type=Path,default=Path('exhibition_tracking_v6.json'))
    ...
    if dynamic_cells(centers,initial_order,H,args.cell_margin) is None: raise SystemExit('FAIL_CLOSED: invalid initial lane cells')
    templates=[]
    ...
    fallback=np.zeros(6,int); ncc_hist=[[] for _ in range(6)]; prev_velocity=np.zeros((6,2),np.float32); min_sep=1e9
    native_done=0
    for step in range(1,steps+1):
        ...
        cells=dynamic_cells(centers,initial_order,H,args.cell_margin)
        ...
        for i in range(6):
            ...
            m,score=template_match_cell(cur_g,templates[i],pred,ylo,yhi,search_x=34*advance,search_y=10*advance); frame_ncc.append(score)
    ...
    cap.release(); median_ncc=np.array([float(np.median(x)) if x else 0.0 for x in ncc_hist]); fb_frac=fallback/max(1,steps)
    quality=[]; okboats=[]
    for i in range(6):
        strong=median_ncc[i]>=args.strong_ncc; lim=args.strong_fallback_frac if strong else args.weak_fallback_frac
    ...
    payload={'video':str(args.video),'slit_sec':args.slit_sec,'fps':fps,'stride':args.stride,'effective_fps':fps/args.stride,
      'leakage_guard':'PRE_RACE_VIDEO_ONLY; JOIN_RESULTS_LATER','method':'v5 LK/template fusion + dynamic vertical lane partition + unchanged confidence fail-closed gate',
      ...
      'median_template_ncc':{str(i+1):round(float(x),3) for i,x in enumerate(median_ncc)},'quality_tier':{str(i+1):quality[i] for i in range(6)},'samples':samples,'accepted':accepted}
    if not accepted: payload['reason']='FAIL_CLOSED: confidence-aware tracking quality insufficient'
        ...
        payload['horizons']=horizons; payload['boats']={str(i+1):{'relative_forward_1_5px':round(float(final_res[i]),3),'stretch_rank':int(ranks[i]),'ses_v6':int(ses[i]),'quality':quality[i]} for i in range(6)}
