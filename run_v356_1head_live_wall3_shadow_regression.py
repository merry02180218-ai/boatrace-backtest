...
EXPECTED_OFFICIAL=['1-3-5','1-3-2','1-2-3']
EXPECTED_SHADOW=['1-4-5','1-4-3','1-2-4']
...
def main():
 ...
 if not z.get('wall3_shadow_eligible') or not z.get('wall3_shadow_applied'): raise RuntimeError(f'shadow not applied {z}')
 if abs(float(z.get('wall3_shadow_risk'))-.47)>1e-9: raise RuntimeError(f'risk drift {z.get("wall3_shadow_risk")}')
 if z.get('wall3_shadow_tickets')!=EXPECTED_SHADOW: raise RuntimeError(f'shadow tickets drift {z.get("wall3_shadow_tickets")}')
 if z.get('result_or_payout_used') is not False or not z.get('chronology_guard'): raise RuntimeError('causality guard failed')
 result={'race_code':z['race_code'],'official_tickets':z['tickets'],'shadow_tickets':z['wall3_shadow_tickets'],'risk':z['wall3_shadow_risk'],'wall_score':z['wall3_shadow_wall_score'],'attack4_score':z['wall3_shadow_attack4_score'],'official_unchanged':True,'result_or_payout_used':False,'AUDIT_OK':True}
