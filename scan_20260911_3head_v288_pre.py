...
def make_current_base():
    y='2026/09/11'; cards=bycode(rows(f'data/programs/race_cards/{y}.csv')); waku=bycode(rows(f'data/programs/waku10/{y}.csv'))
    bias=learn_bias(); out=[]
    # PRE means no target-day exhibition feeds are supplied at all.
    for code,card in cards.items():
        ...
        # Preserve human-readable names if present.
        for b in range(1,7):
            for key in (f'艇{b}_選手名',f'艇{b}_選手登番'):
                if key in card: z[key]=card.get(key)
        out.append(z)
    if not out: raise RuntimeError('no current PRE rows built')
...
def main():
    ...
    # Reproduce v243 monthly head-candidate gate on the currently available September universe.
    base_te,_=v223.fit_head(d,list(basefs),vc,FIRST,NEXT)
    k=max(1,int((base_te._p>=.30).sum()))
    te,_=v223.fit_head(d,fs,vc,FIRST,NEXT)
    te=te[te._date==pd.Timestamp(DAY)].copy()
    ...
    hist.date=hist.date.astype(str); hist['_target']=v249.target(hist).astype(int)
    cs=v249.cols(hist)
    ...
    for idx,r in sel.iterrows():
        ...
        code=str(r.race_code).zfill(12); jcd=int(code[8:10]); rno=int(code[10:12])
        rec.append(dict(race_code=code,jcd=jcd,rno=rno,pre_grade=c.grade,pre_score=float(c._score),pre_pct=float(c._pct),p3=float(r._p),
                        b3_minus_b2_motor=float(r.get('c_b3_minus_b2_motor',np.nan)),b3_inside_nst=float(r.get('c_b3_inside_nst',np.nan))))
    q=pd.DataFrame(rec).sort_values(['pre_grade','pre_pct'],ascending=[False,False]) if rec else pd.DataFrame(columns=['race_code'])
    q.to_csv(OUT,index=False)
    L=['# 2026-09-11 3-head v288 PRE scan','',
       '- Result-blind: target-day result/payout endpoints are not used.','- PRE-only: target-day exhibition/ST/original-exhibition feeds are intentionally not supplied.','- Operational bridge: historical v249 uses month-internal percentile; on 2026-09-11 this scan ranks within the currently available target-day v243 candidate universe because future September races do not exist yet.','',f'- v243 current head universe: {len(sel)}',f'- PRE S+A candidates: {len(q)}','', '|race|grade|pct|p3|b3-b2 motor|b3 inside NST|','|---|---|---:|---:|---:|---:|']
    for _,x in q.iterrows():
        L.append(f"|JCD{x.jcd:02.0f} {x.rno:.0f}R|{x.pre_grade}|{x.pre_pct:.3f}|{x.p3:.3f}|{x.b3_minus_b2_motor:.3f}|{x.b3_inside_nst:.3f}|")
