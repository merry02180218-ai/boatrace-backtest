...
def main():
    ...
    L=[f'# {DAY.isoformat()} 3-head v288 PRE scan','', '- Result-blind: target-day result/payout endpoints are not used.','- PRE-only: target-day exhibition/ST/original-exhibition feeds are intentionally not supplied.','- Current PRE cards/Waku10 are supplied by the caller; for production daily runs, source/fallback coverage and fail-closed exclusions are recorded in input_manifest.json.','- Leakage-free operational PRE: score cutpoints are frozen from the prior-history training universe only; current-day/future races do not define the S/A thresholds.',f'- S score threshold (training 70th percentile): {s_thr:.6f}',f'- A score threshold (training 20th percentile): {a_thr:.6f}','',f'- v243 current head universe: {len(sel)}',f'- PRE S+A candidates: {len(q)}','', '|race|grade|train-CDF pct|p3|b3-b2 motor|b3 inside NST|','|---|---|---:|---:|---:|---:|']
