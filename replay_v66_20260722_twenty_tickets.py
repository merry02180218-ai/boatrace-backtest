...
OUT='summary_v66_20260722_twenty_tickets.md'
...
def main():
    ...
    for r in cand:
        ...
        rank=next((j for j,z in enumerate(cc,1) if z[3]==actual),None) if actual else None
        L+=['',f"結果: **{actual or '-'}** / 頭的中: **{'○' if int(float(r.get('head_hit') or 0)) else '×'}** / 20通り内順位: **{rank if rank else '-'}位**"]
