...
Select N with maximum shadow_EV_multiple. This is NOT production-adopted until historical/
prospective validation. Output contains the full candidate table and suggested stakes.
"""
...
from predict_v192_3head_live_manual import target,fit_head,exact_prior_bias,build_manual_maps
...
def calc_candidate(n,ordered,masses,odds,p3):
    ...
    tickets=[{'combo':t,'odds':float(v),'stake':int(s),'equal_return':float(v*s)} for t,v,s in zip(ts,vals,stakes) if int(s)>0]
    return {'points':n,'composite_odds':comp,'v166_mass':mass,'estimated_hit_prob':est_hit,'shadow_ev_multiple':ev,'tickets':tickets}
...
def main():
    ...
    if p<CUT:
        res.update({'v166_lambda':1.0,'rank20':[],'odds_snapshot':None,'point_candidates':[],'shadow_choice':None})
    else:
        hist=[]
        with SRC.open(encoding='utf-8-sig',newline='') as f:
            for r in csv.DictReader(f):
                if r.get('date','')<hd.isoformat():hist.append(r)
        pm,pair_n=fit_pair(hist);ordered,effn,ent,m0=pair_distribution(fr,pm)
        # pair_distribution only precomputes fixed masses; recompute all N=4..15.
        # Recover normalized probabilities from successive model scores directly in exact rank order.
        ...
        choice=max(cs,key=lambda c:(c['shadow_ev_multiple'],-c['points'])) if cs else None
    ...
    L=[f"# v218 SHADOW live odds-aware 3-head {hd} {venue} {rno}R",'',f"- boat3: **{res['boat3']}**",f"- v165 p3head: **{100*p:.2f}%**",f"- decision: **{res['decision']}** (production cut 30%)",'- production point rule is NOT changed; odds-aware point choice below is SHADOW only.']
    if res['decision']=='BUY':
        s=res['odds_snapshot'];L += [f"- odds source: **{s['source']}**",f"- odds fetched: **{s['fetched_at_jst']}**",'','## TopN candidates','|N|composite|v166 mass|est hit|shadow EV|','|---:|---:|---:|---:|---:|']
        for c in res['point_candidates']:L.append(f"|{c['points']}|{c['composite_odds']:.3f}|{100*c['v166_mass']:.1f}%|{100*c['estimated_hit_prob']:.2f}%|{c['shadow_ev_multiple']:.3f}|")
        c=res['shadow_choice'];L += ['','## SHADOW choice',f"- points: **{c['points']}**",f"- composite odds: **{c['composite_odds']:.3f}**",f"- shadow EV multiple: **{c['shadow_ev_multiple']:.3f}**",'','|combo|odds|stake|approx equal return|','|---|---:|---:|---:|']
