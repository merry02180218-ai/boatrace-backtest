...
def main():
 ...
 for mon in MONTHS:
  ...
  for _,r in d[(d._date<first)&(d._y==1)].iterrows():
   ts=v222.order(r,pair,'V221');act=v222.v166.combo(r.get('actual_combo'));a='-'.join(map(str,act)) if len(act)==3 else '';vals.append(int(a in ts[:10]))
  ...
  for _,r in sel.iterrows():
   ...
   o=oi.loc[code];o=o.iloc[-1] if isinstance(o,pd.DataFrame) else o;ts=v222.order(r,pair,'V221');tickets=ts[:10]
