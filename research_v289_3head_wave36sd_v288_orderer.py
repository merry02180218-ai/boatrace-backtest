...
def main():
    ...
    raw=pd.read_csv(LEG,dtype={'race_code':str});dc=v165.pc(raw,['date','race_date','ymd']);raw['_date']=pd.to_datetime(raw[dc].astype(str),errors='coerce');raw=raw[raw._date.notna()&(raw._date<pd.Timestamp('2026-07-01'))].copy()
    # Rebuild the exact prior-only V221 history/player features. No imputation/substitute orderer.
    hist=v221.build(raw,dc);hist['race_code_norm']=hist.race_code.astype(str).str.zfill(12)
    sm=src.copy();sm['race_code_norm']=sm.race_code.astype(str).str.zfill(12);sidx=sm.set_index('race_code_norm',drop=False)
    rec=[];missing=[]
    for mo in MONTHS:
        first=pd.Timestamp(mo+'-01');nextm=first+pd.offsets.MonthBegin(1);tr=hist[hist._date<first]
        ...
        for _,r in targets.iterrows():
            ...
            if code not in sidx.index:missing.append(code);continue
            sr=sidx.loc[code];sr=sr.iloc[-1] if isinstance(sr,pd.DataFrame) else sr
            odds=parse_odds(sr.get('closing_odds__json',''))
            if odds is None:missing.append(code);continue
            ts=v242.safe_order(r,pair)
            if ts is None:missing.append(code);continue
    ...
    q=pd.DataFrame(rec);expected=len(sel);covered=len(q);missing_sel=sorted(sel_codes-set(q.race_code.astype(str))) if len(q) else sorted(sel_codes)
    out={'wave':'36S-D-v288-orderer','orderer':'exact V221 ordered-pair ranker + v242 variable TopN 5-10 + exact JPY10000 Dutch','wave36sc_expected_races':expected,'covered_races':covered,'missing_or_invalid_count':len(missing_sel),'missing_or_invalid_examples':missing_sel[:30],'monthly':{mo:metric(q[q.month==mo]) for mo in MONTHS},'apr_jun':metric(q),'v288_overlap':int(q.race_code.astype(str).isin(set(pd.read_csv(BC,dtype=str).race_code.astype(str))).sum()),'september_forbidden':True}
    if out['v288_overlap']:raise RuntimeError('v288 overlap')
    # Exact parity experiment must be complete; fail closed rather than silently comparing a partial sample.
