#!/usr/bin/env python3
"""v189 diagnostic: pre-exhibition monitoring layer for canonical v165 -> v166 3-head production.

This revision diagnoses the historical alignment before training. It never fabricates a
formal p3head. Production remains actual v165 p3head>=.30 -> v166 Top10 after exhibition.
"""
...
TARGET_RECALL=.85
PRE_FS=['one_grade','one_wr','one_local','one_motor','one_waku_wr','one_nst_strength','one_waku_sr_strength','one_past_win','one_meet_st_strength','one_score','threat2','threat3','threat4','threat5','threat6','threat23_max','threat_all_max','margin2','margin3','margin23','margin_all']
...
def pipe():
    pre=ColumnTransformer([('n',StandardScaler(),PRE_FS),('v',OneHotEncoder(handle_unknown='ignore'),['venue'])])
    return Pipeline([('pre',pre),('lr',LogisticRegression(C=.35,max_iter=1800,class_weight='balanced'))])

def frame(rs):return pd.DataFrame([{**{k:ff(r.get(k),0) for k in PRE_FS},'venue':str(r.get('venue','')).zfill(2)} for r in rs])
def fit(rs):
...
def metrics(rs,pp,cut):
    y=np.asarray([ii(r['formal_v165_candidate']) for r in rs]);sel=pp>=cut;pos=int(y.sum());tp=int((sel&(y==1)).sum());nsel=int(sel.sum())
    return {'R':len(rs),'positives':pos,'selected':nsel,'shortlist_rate':nsel/len(rs) if rs else 0,'recall':tp/pos if pos else 0,'precision':tp/nsel if nsel else 0}
def choose_cut(rs,pp):
    y=np.asarray([ii(r['formal_v165_candidate']) for r in rs]);pos=int(y.sum())
    if not pos:return .5
    good=[]
    for t in sorted(set(float(x) for x in pp)):
        sel=pp>=t;rec=float((sel&(y==1)).sum())/pos
        if rec>=TARGET_RECALL:good.append(t)
    return max(good) if good else float(min(pp))

def fetch_hist(d):
    y=d.strftime('%Y/%m/%d');return d,rows(f'data/programs/race_cards/{y}.csv'),rows(f'data/programs/waku10/{y}.csv')

def build_historical_pre():
    src=pd.read_csv(SRC);lab=pd.read_csv(V165)
    wanted=[]
    for _,z in lab.iterrows():
        ix=int(z['source_index'])
        if ix<0 or ix>=len(src):continue
        # source_index in v165 is the original pandas index, so use loc when possible.
        try:s=src.loc[ix]
        except Exception:s=src.iloc[ix]
        rc=s.get('race_code','')
        if pd.isna(rc):rc=''
        else:
            try:rc=str(int(float(rc)))
            except Exception:rc=str(rc).strip()
        wanted.append({'source_index':ix,'date':str(z['date'])[:10],'month':str(z['month']),'race_code':rc,'p3head':float(z['p3head']),'y3head':int(z.get('y3head',0)),'formal_v165_candidate':int(float(z['p3head'])>=.30)})
    days=sorted({date.fromisoformat(z['date']) for z in wanted});got={}
    ...
    for d,(cards,wm) in got.items():
        z={}
        for c in cards:
            rc=str(c.get('レースコード','')).strip();z[rc]=(c,wm.get(rc))
        byday[d]=z
    out=[];miss=0;sample_miss=[];zeros={b:0.0 for b in range(1,7)}
    for z in wanted:
        d=date.fromisoformat(z['date']);pair=byday.get(d,{}).get(z['race_code'])
        if not pair or pair[1] is None:
            miss+=1
            if len(sample_miss)<10:sample_miss.append((z['date'],z['race_code'],len(byday.get(d,{}))))
            continue
        card,w=pair
    ...
    return out,miss,len(wanted),sample_miss
...
def racer_names():
    txt=get_text('https://raw.githubusercontent.com/ryuriki/boatrace-v2/main/config/racers_name.yaml');out={}
...
def parse_race(jcd,rno,names):
    url=f'https://www.boatrace.jp/owpc/pc/race/racelist?rno={rno}&jcd={jcd}&hd={HD8}';soup=BeautifulSoup(get_text(url),'html.parser');tables=soup.find_all('div',class_='table1')
    if len(tables)<2:raise ValueError('racelist table missing')
    tds=tables[0].find('tbody').find_all('td');deadline=tds[rno].get_text(strip=True) if len(tds)>rno else '';bodies=tables[1].find_all('tbody')
    if len(bodies)<6:raise ValueError('lane bodies missing')
    lanes={}
    for lane in range(1,7):
        cells=bodies[lane-1].find_all('td');info=cells[2].find_all('div',class_='is-fs11');racer=info[0].get_text(strip=True);rid=ii(racer.split('/')[0].strip());grade=racer.split('/')[1].strip() if '/' in racer else ''
        ...
        lanes[lane]={'rid':rid,'name':names.get(rid,str(rid)),'grade':grade,'nst':ff(c3[2] if len(c3)>2 else .20,.20),'wr':ff(zn[0] if zn else 0),'local':ff(jl[0] if jl else 0),'motor2':ff(mo[1] if len(mo)>1 else 0),'motor3':ff(mo[2] if len(mo)>2 else 0)}
...
def runtime_scan(final_model,cut):
    ...
    out.sort(key=lambda z:(z['deadline'] or '99:99',z['venue'],z['race']));return out,active,errs

def main():
    hist,miss,wanted,sample_miss=build_historical_pre();june=[r for r in hist if r['month']=='2026-06'];july=[r for r in hist if r['month']=='2026-07'];aug=[r for r in hist if r['month']=='2026-08']
    counts={m:(len(z),sum(ii(r['formal_v165_candidate']) for r in z)) for m,z in [('2026-06',june),('2026-07',july),('2026-08',aug)]}
    print('v189 join diagnostic wanted',wanted,'hist',len(hist),'miss',miss,'monthly',counts,'sample_miss',sample_miss,flush=True)
    if not hist or min(v[1] for v in counts.values())<2:
        SUMMARY.write_text('# v189 PRE diagnostic\n\n'+f'- wanted: {wanted}\n- joined: {len(hist)}\n- missing: {miss}\n- monthly rows/positives: {counts}\n- sample missing: {sample_miss}\n- status: **FAIL_JOIN_DIAGNOSTIC**\n',encoding='utf-8')
        raise SystemExit('too few formal positives after diagnostic')
    m1=fit(june);pv=pred(m1,july);cut=choose_cut(july,pv);mv=metrics(july,pv,cut);pt=pred(m1,aug);mt=metrics(aug,pt,cut);passed=mt['recall']>=.80 and mt['shortlist_rate']<=.35
    ...
    pd.DataFrame(july+aug).to_csv(OUT_HIST,index=False,encoding='utf-8-sig');mf=fit(hist);runtime,active,errs=runtime_scan(mf,cut);pd.DataFrame(runtime).to_csv(OUT_PRE,index=False,encoding='utf-8-sig');cand=[z for z in runtime if z['pre_watch']];now=datetime.now(ZoneInfo('Asia/Tokyo'))
    L=['# v189 3号艇 v165-aligned PRE monitor','','**PREは監視候補でありBUYではない。正式候補は展示後の actual v165 p3head>=30% のみ。**','',f'- historical PRE rows: **{len(hist):,}R** / join missing **{miss}R**',f'- July validation: R {mv["R"]}, formal {mv["positives"]}, watch {mv["selected"]} ({100*mv["shortlist_rate"]:.1f}%), recall {100*mv["recall"]:.1f}%, precision {100*mv["precision"]:.1f}%',f'- August untouched: R {mt["R"]}, formal {mt["positives"]}, watch {mt["selected"]} ({100*mt["shortlist_rate"]:.1f}%), recall {100*mt["recall"]:.1f}%, precision {100*mt["precision"]:.1f}%',f'- PRE validation gate: **{"PASS" if passed else "FAIL"}**','',f'## 2026-09-09 PRE watch: {len(cand)}R',f'- scan JST: {now:%Y-%m-%d %H:%M:%S}','|締切|場|R|3号艇|級|PRE p|','|---|---|---:|---|---|---:|']
