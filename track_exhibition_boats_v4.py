#!/usr/bin/env python3
"""Template-assisted six-boat tracker for low-latency live exhibition SES.

Result-blind research/live helper.

Changes from v3:
- keeps arbitrary exhibition-entry order support
- fuses local LK motion with bounded template matching around a predicted center
- adapts templates only on high-confidence matches
- records per-boat template confidence and fallback counts
- remains fail-closed when identity/tracking quality is insufficient
"""
...
def local_motion(prev_g,cur_g,cx,cy,roi_w=76,roi_h=42):
    mask=np.zeros_like(prev_g)
    x1=max(0,int(round(cx-roi_w*0.45))); x2=min(prev_g.shape[1],int(round(cx+roi_w*0.55)))
    ...
    kw=dict(winSize=(23,23),maxLevel=3,criteria=(cv2.TERM_CRITERIA_EPS|cv2.TERM_CRITERIA_COUNT,30,.01))
    ...
    fb=np.linalg.norm(pbv-p0v,axis=1)
    d=p1v-p0v
    mag=np.linalg.norm(d,axis=1)
    good=(st[:,0]==1)&(stb[:,0]==1)&np.isfinite(fb)&(fb<1.8)&(mag<24)
    d=d[good]
    if len(d)<4: return None,int(len(d)),None
    med=np.median(d,axis=0)
    radial=np.linalg.norm(d-med,axis=1)
    rmed=float(np.median(radial))
    keep=radial < max(3.0,3.5*rmed)
    d=d[keep]
    if len(d)<4: return None,int(len(d)),None
    med=np.median(d,axis=0)
...
def template_match(cur_g, template, pred, search_x=42, search_y=15):
    ...
    cx=x1+loc[0]+tw/2; cy=y1+loc[1]+th/2
    return np.array([cx,cy],np.float32),float(score)
...
def main():
    ...
    ap.add_argument('--out',type=Path,default=Path('exhibition_tracking_v4.json'))
    ...
    ap.add_argument('--max-fallback-frac',type=float,default=.35)
    args=ap.parse_args()
    ...
    for fi in range(1,nmax+1):
        ...
        lk=[]; ns=[]; spreads=[]
        for cx,cy in centers:
            d,n,s=local_motion(prev_g,cur_g,float(cx),float(cy)); lk.append(d); ns.append(n); spreads.append(s)
        ...
        for i in range(6):
            d=lk[i]
            if d is not None and np.linalg.norm(d-fleet)<=14:
                pred=centers[i]+d
            elif np.linalg.norm(prev_velocity[i])>0:
                pred=centers[i]+prev_velocity[i]
            else:
                pred=centers[i]+fleet
            m,score=template_match(cur_g,templates[i],pred)
            frame_ncc.append(score)
            if m is not None and score is not None and score>=args.min_ncc and abs(float(m[1]-pred[1]))<=16:
                # Fuse template position with motion prediction. Template dominates x identity;
                # prediction damps occasional water/wake matches.
                est=(0.72*m + 0.28*pred).astype(np.float32)
                if np.linalg.norm(est-centers[i])>32:
                    ...
                    if score>=0.72:
                        ...
                        if nt is not None and nt.shape==templates[i].shape:
                            templates[i]=cv2.addWeighted(templates[i],0.85,nt,0.15,0)
            else:
                fallback[i]+=1; est=pred.astype(np.float32)
            new[i]=est
        velocities=new-centers; centers=new; prev_velocity=0.65*prev_velocity+0.35*velocities
        ...
        if not np.all(np.diff(ys)>4): raise SystemExit(f'FAIL_CLOSED: lane separation too small at frame {fi}')
        prev_g=cur_g
    ...
    max_fb=max(8,int(args.max_fallback_frac*nmax))
    median_ncc=[float(np.median(x)) if x else 0.0 for x in ncc_hist]
    accepted=bool(np.all(fallback<=max_fb) and np.sum(np.array(median_ncc)>=args.min_ncc)>=5)
    payload={'video':str(args.video),'slit_sec':args.slit_sec,'fps':fps,
             'leakage_guard':'PRE_RACE_VIDEO_ONLY; JOIN_RESULTS_LATER',
             'method':'LK prediction + bounded adaptive template match + arbitrary entry-order guard',
             'entry_order':entry_order,
             'template_fallbacks':{str(i+1):int(x) for i,x in enumerate(fallback)},
             'max_allowed_fallbacks_per_boat':int(max_fb),
             'median_template_ncc':{str(i+1):round(float(x),3) for i,x in enumerate(median_ncc)},
             'samples':samples,'accepted':accepted}
    if not accepted:
        payload['reason']='FAIL_CLOSED: template/LK tracking quality insufficient'
    else:
        ...
        payload['boats']={str(i+1):{'relative_forward_1_5px':round(float(final_res[i]),3),
                                    'stretch_rank':int(ranks[i]),'ses_v4':int(ses[i])} for i in range(6)}
        payload['ses_scale_px']=round(float(scale),3)
        payload['warning']='EXPERIMENTAL live score; validate across venues before production weighting.'
