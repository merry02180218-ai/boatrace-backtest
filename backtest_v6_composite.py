...
from backtest_v5_ev import TRAIN_START,TRAIN_END,TEST_START,TEST_END,PRELOAD_START,EV_MIN,HEAD_SCORE_MIN,cal_prob,market_conditional,daycat,process_features
from collections import defaultdict
from datetime import timedelta

MIN_COMPOSITE=5.0
MAX_TICKETS_PER_RACE=12
...
def main():
    ...
    while d<=TEST_END:
        ...
        for r,x,s4,s5,dc in feats:
            ...
            chosen=[]
            for z in cand:
                if len(chosen)>=MAX_TICKETS_PER_RACE: break
                trial=chosen+[z]
    ...
    if bets:
        with open('bets_v6.csv','w',newline='',encoding='utf-8-sig') as f1:
            w=csv.DictWriter(f1,fieldnames=list(bets[0].keys()));w.writeheader();w.writerows(bets)
    if races:
        with open('races_v6.csv','w',newline='',encoding='utf-8-sig') as f1:
    ...
    L=['# 2026-08-03〜2026-09-02 v6 合成オッズ5倍ルール','',
       '校正は6/1〜8/2のみ。テスト期間の結果は買い目固定後に読み込み。',
       f'固定ルール: 4/5頭score>=60、個別EV>={EV_MIN}、EV順に買い目を追加し、4頭+5頭を合わせた合成オッズが{MIN_COMPOSITE}倍以上の範囲で最大{MAX_TICKETS_PER_RACE}点。1点100円。','',
       '|区分|購入点数|的中|投資|払戻|回収率|','|---|---:|---:|---:|---:|---:|']
    for name,bs in [('全体',bets),('4頭',[x for x in bets if x['head']==4]),('5頭',[x for x in bets if x['head']==5])]:
        n,h,st,ret,roi=agg(bs); L.append(f'|{name}|{n}|{h}|{st:,}円|{ret:,}円|{roi:.1f}%|')
    dual_codes={r['race_code'] for r in races if r['heads_bet']=='45'}
    db=[x for x in bets if x['race_code'] in dual_codes]; n,h,st,ret,roi=agg(db); L.append(f'|4+5両展開レース|{n}|{h}|{st:,}円|{ret:,}円|{roi:.1f}%|')
    L+=['','## レース単位','|項目|値|','|---|---:|',f'|購入レース数|{len(races)}|',f'|両展開購入レース|{len(dual_codes)}|',f'|平均点数/レース|{(len(bets)/len(races) if races else 0):.2f}|',f'|平均合成オッズ|{(sum(r["composite_odds"] for r in races)/len(races) if races else 0):.2f}倍|']
    L+=['','## 日次別','|日次|点数|的中|回収率|','|---|---:|---:|---:|']
    for dc in ['初日','2日目','3日目以降']:
        bs=[x for x in bets if x['day_cat']==dc]; n,h,st,ret,roi=agg(bs); L.append(f'|{dc}|{n}|{h}|{roi:.1f}%|')
    L+=['','## 注意','- 合成オッズ=1/Σ(1/各買い目オッズ)。締切約5分前od3で計算。','- 払戻は確定払戻。','- 点数上限12は安全弁で、基本的には合成5倍条件が先に効く。']
    open('summary_v6.md','w',encoding='utf-8').write('\n'.join(L)+'\n')
