...
ONE_D = list(FEATURES)
SOURCE_FOR_FEATURE = {
    'one_ex':'has_tkz','ex_margin23':'has_tkz',
    'one_st':'has_stt',
    'one_lap':'has_orig','one_turn':'has_orig','one_straight':'has_orig','one_orig_avg':'has_orig',
    'turn_margin23':'has_orig','straight_margin23':'has_orig',
}
...
def source_ready(df:pd.DataFrame, feat:str) -> pd.Series:
    src=SOURCE_FOR_FEATURE[feat]
    return df[src].fillna(0).eq(1) & df[feat].notna()

def all_sources_ready(df:pd.DataFrame) -> pd.Series:
    return df[['has_tkz','has_stt','has_orig']].fillna(0).eq(1).all(axis=1) & df[FEATURES].notna().all(axis=1)
...
def prepare() -> pd.DataFrame:
    ...
    if len(x)!=345 or int(x.hit.sum())!=139 or int(x.head_hit.sum())!=290:
        raise AssertionError(f'v320 reconcile failed R={len(x)} exact3={int(x.hit.sum())} head={int(x.head_hit.sum())}')
    v=load_v108_subset(set(x.race_code))
    if v.empty: raise AssertionError('v108 join subset empty')
    if v.race_code.duplicated().any(): v=v.drop_duplicates('race_code',keep='last')
    ...
    y['all_complete']=all_sources_ready(y).astype(int)
    y.to_csv(OUT/'v325_joined_dev.csv',index=False)
    return y

def eval_rule(df, feat, op, thr):
    ready=source_ready(df,feat)
    z=df[ready]
    p=z[z[feat]>=thr] if op=='>=' else z[z[feat]<=thr]
    m=metrics(p)
    m.update({'eligible_source_R':len(z),'kind':'1d','feature':feat,'op':op,'threshold':thr})
    return m

def discover_1d(disc:pd.DataFrame, val:pd.DataFrame):
    cand=[]
    for feat in ONE_D:
        dready=disc[source_ready(disc,feat)]
        vals=dready[feat].dropna().to_numpy(float)
        if len(vals)<20: continue
        qs=np.unique(np.quantile(vals,np.arange(.10,.91,.05)))
        for op in ('>=','<='):
            for t in qs:
                dm=eval_rule(disc,feat,op,float(t)); vm=eval_rule(val,feat,op,float(t))
...
def logistic_candidate(disc:pd.DataFrame,val:pd.DataFrame):
    tr=disc[all_sources_ready(disc)].copy(); va=val[all_sources_ready(val)].copy()
    if len(tr)<30 or len(va)<10:return pd.DataFrame(),None,None
    model=Pipeline([('sc',StandardScaler()),('lr',LogisticRegression(C=.25,max_iter=2000))])
    ...
    for t in np.unique(np.quantile(pdsc,np.arange(.10,.91,.05))):
        ...
        rows.append({'kind':'logit','threshold':float(t),'disc_eligible_source_R':len(tr),'val_eligible_source_R':len(va),
...
def apply_candidate(df,best,model=None):
    ...
    if best['kind']=='1d':
        feat=best['feature']; z=df[source_ready(df,feat)].copy()
        return z[z[feat]>=float(best['threshold'])] if best['op']=='>=' else z[z[feat]<=float(best['threshold'])]
    z=df[all_sources_ready(df)].copy()
...
def main():
    ...
    print('JOIN',int(y.joined.sum()),'tkz',int((y.has_tkz==1).sum()),'stt',int((y.has_stt==1).sum()),'orig',int((y.has_orig==1).sum()),'all_complete',int(y.all_complete.sum()))
    print('BASE',metrics(y))
    monthly=[]
    for m,g in y.groupby('month'):
        mm=metrics(g);monthly.append({'month':m,'scope':'base',**mm});print('BASE_MONTH',m,mm)

    disc=y[y.month.isin(['2026-02','2026-03','2026-04'])].copy()
    val=y[y.month.eq('2026-05')].copy(); fwd=y[y.month.eq('2026-06')].copy()
    print('SPLITS',metrics(disc),metrics(val),metrics(fwd))
    print('SOURCE_COMPLETE_SPLITS',int(all_sources_ready(disc).sum()),int(all_sources_ready(val).sum()),int(all_sources_ready(fwd).sum()))

    c1,b1=discover_1d(disc,val); cl,bl,model=logistic_candidate(disc,val)
    c1.to_csv(OUT/'v325_candidates_1d.csv',index=False); cl.to_csv(OUT/'v325_candidates_logit.csv',index=False)
    candidates=[]
    ...
    if chosen is None:
        ...
        summary={'candidate':'NONE','reason':'No fail-closed predeclared candidate met validation freeze criteria'}
    else:
        typ,best,mdl=chosen
        ...
        print('JUN_FORWARD_PASS',fm); print('JUN_FORWARD_SKIP',sm)
        for m,g in y.groupby('month'):
    ...
    with (OUT/'summary_v325_1head_exhibition_postfilter.md').open('w',encoding='utf-8') as f:
        f.write('# v325 1HEAD exhibition postfilter (fail-closed)\n\n')
        ...
        f.write('- Missing required exhibition source => automatic SKIP; neutral 0.5 defaults are never accepted as source evidence.\n')
        for k,v in summary.items(): f.write(f'- {k}: {v}\n')
        f.write('\nJul/Aug are intentionally not read/evaluated. Run reference only after a development candidate is frozen and Jun-forward-supported.\n')
