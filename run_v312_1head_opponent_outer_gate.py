...
def gate_table(sl):
    """One row/race. Features are safe within-race summaries; label used only in training."""
    fs=v311.audited_features(sl)
    # Bound dimension and keep interpretable safe source families.
    keep=[]
    for c in fs:
        fam=v311.family(c)
        if fam in {'HEADRISK','MOTOR','PLAYER','FORM_STATIC','POSITION','OTHER_SAFE'}:
            keep.append(c)
    # Prefer raw candidate/role + headrisk and cap to avoid huge unstable aggregate space.
    keep=keep[:120]
    rows=[]
    for code,g in sl.groupby('race_code',sort=False):
        ...
        for c in keep:
            ...
            inn=x[b.isin([2,3])]; out=x[b.isin([4,5,6])]
...
def gate_predict(gt,tm,C=0.3):
    tr=gt[gt.month<tm].copy(); te=gt[gt.month==tm].copy()
    # Train only races with a valid 1-x-y SECOND label in the opponent training universe.
    tr=tr[tr.actual2.isin([2,3,4,5,6])]
    meta={'race_code','month','date','actual2','y_outer'}
    fs=[c for c in gt.columns if c not in meta and 'meet_' not in c]
    ...
    m.fit(tr[fs],tr.y_outer.astype(int))
...
def reweight(base,pg,mult,threshold):
    q={int(k):float(v) for k,v in base.items()}
    boost=1.0
    if pg>=threshold:
        boost=mult
    for b in (4,5,6): q[b]=q.get(b,0.0)*boost
    s=sum(q.values())
    return {b:(q[b]/s if s>0 else 0.2) for b in sorted(q)}
...
def main():
    ...
    # v311 best base construction: FULL_HEADRISK with START family dropped.
    ...
    for name,mult,th in configs:
        ...
        for tm in TM:
            ...
            for code,bp in base[tm].items():
                p2[tm][code]=bp if name=='DROP_START_BASE' else reweight(bp,gates[tm].get(code,0.0),mult,th)
    ...
    # Prefer overall gain, then stability, then outer recovery, then exact3.
