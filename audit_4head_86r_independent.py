...
Rebuilds the head population from source lineage, not from combo-study output.
September outcomes are never read.  Head-rate coverage and archived-odds ROI
coverage are reported separately.  ROI is retrospective evidence only.
"""
...
def rebuild():
    # Independent source reconstruction through the shared causal feature builders.
    z=ex.build_base()
    x=ex.build_ex(z)
    x=x[(x.motor_win_diff_4v3>=WIN)&(x.motor_2ren_diff_4v3>=REN2)&
        (x.player4_all_win>=PLAYER)&(x.basic_complete==1)&
        (x.st4_adv_inside>=ST)&(x.orig_avg_available==1)&
        (x.orig4_adv_inside>=ORIG)].copy()
    x['race_code']=x.race_code.astype(str).str.zfill(12)
    x['date']=x.date.astype(str)
    x['month']=x.date.str[:7]
    x=x[(x.date>='2026-04-01')&(x.date<='2026-08-31')].copy()
    tr=x[x.month.isin(['2026-04','2026-05','2026-06'])]
    ho=x[x.month.isin(['2026-07','2026-08'])]
    assert (len(tr),int(tr.head4.sum()))==(86,35),(len(tr),int(tr.head4.sum()))
    assert (len(ho),int(ho.head4.sum()))==(78,35),(len(ho),int(ho.head4.sum()))
    print('HEAD4_86R_INDEPENDENT_REPLAY_OK')
    return x


def pair_orders(rs):
    """Historical prior-only opponent ranking lineage for ROI audit.

    This is deliberately kept separate from head reconstruction.  The current
    production v283 contract is reported in metadata; archived historical ROI
    remains retrospective and does not claim prospective pre-deadline snapshots.
    """
    out={}
    for mon in [f'2026-{m:02d}' for m in range(4,9)]:
        ...
        for r in rs:
            ...
            out[str(r.get('race_code','')).zfill(12)]=ps[:4]
    return out


def actual_map(rs):
    return {str(r.get('race_code','')).zfill(12):(ii(r.get('winner')),ii(r.get('second')),ii(r.get('third')),ii(r.get('valid_result'))) for r in rs}
...
def summarize_head(x):
    rows=[]
    for mon,g in x.groupby('month'):
        rows.append({'period':mon,'R':len(g),'head4':int(g.head4.sum()),'head4_rate_pct':100*g.head4.mean()})
...
def main():
    ...
    for _,r in x.iterrows():
        code=r.race_code; rec={'date':r.date,'month':r.month,'race_code':code,'head4':int(r.head4),'roi_covered':0,'hit':np.nan,'payout':np.nan}
        ps=orders.get(code); a=am.get(code)
        if ps and a and a[3]==1 and not oi.empty and code in oi.index:
            o=oi.loc[code]; o=o.iloc[-1] if isinstance(o,pd.DataFrame) else o
            ts=[f'4-{s}-{t}' for s,t in ps]
            st=settle(ts,o,a)
            if st is not None:
                rec.update(roi_covered=1,hit=st[0],payout=st[1])
        rr.append(rec)
    rd=pd.DataFrame(rr);rd.to_csv(OUT/'roi_detail.csv',index=False)
    sm=[]
    ...
    for name,mons in periods:
        allg=rd[rd.month.isin(mons)]; g=allg[allg.roi_covered==1]
        stake=len(g)*BANK;payout=float(g.payout.sum()) if len(g) else 0
    ...
    meta={'candidate':'HEAD4_86R_ORIGAVG_RESEARCH','thresholds':{'motor_win_diff_4v3':WIN,'motor_2ren_diff_4v3':REN2,'player4_all_win':PLAYER,'st4_adv_inside':ST,'orig4_adv_inside':ORIG},'head_replay_assert':'86/35 Apr-Jun; 78/35 Jul-Aug','roi_note':'retrospective archived-odds audit; separate coverage; not prospective pre-deadline evidence','ticket_stake':'4-head fixed, Top4 historical opponent audit, 10000 yen inverse-odds Dutch / 100-yen Hamilton','production_v283_contract':'TOP2XTOP2 alpha2=.60; production unchanged','september':'UNREAD','production':'HEAD4_V291_COMP7 unchanged'}
