...
def main():
 ...
 out.update({'status':'PASS' if head_pass else 'DROP','attention_level':'WATCH' if watch_pass else ('BASIC' if head_pass else 'DROP'),'watch_pass':bool(watch_pass),'production_profile':prod.PROFILE_NAME,'live_operation_profile':prod.LIVE_OPERATION_PROFILE_NAME,'watch_profile':prod.WATCH_PROFILE_NAME,'final_head_p':ph,'head_cutoff':prod.LIVE_HEAD_CUTOFF,'opp_mass':mass,'opponent_mass_min':prod.LIVE_OPPONENT_MASS_MIN,'watch_opponent_mass_min':prod.WATCH_OPPONENT_MASS_MIN,'live_env_w':prod.LIVE_EXHIBITION_ENV_W,'live_q':prod.LIVE_EXHIBITION_Q,'head_exhibition_pass':bool(x['head_exhibition_pass']),'tickets':tickets,'opponent_core':core,'result_or_payout_used':False,'chronology_guard':True,'finalized':True,'scoring_ms':(time.perf_counter()-t0)*1000})
