...
def main()->int:
    ...
    try:
        ...
        audit=v326.raw_completeness(tkz,stt,orig);ready=all(audit[k] for k in ('tkz_all6','stt_all6','orig_turn_all6','orig_straight_all6','orig_avg_all6'))
        obj={'race_code':code,'ready':bool(ready),'audit':audit,'fetched_at_jst':datetime.now(JST).isoformat(),'sha256':{k:hashlib.sha256(v.encode()).hexdigest() for k,v in bodies.items()},'tkz':tkz,'stt':stt,'orig':orig,'result_or_payout_used':False}
