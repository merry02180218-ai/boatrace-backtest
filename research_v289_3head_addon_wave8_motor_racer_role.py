#!/usr/bin/env python3
"""v289 Wave 8: motor/racer-regime-conditioned 2nd/3rd role models.

Final-v288-NO_BET candidates are split into four pre-race regimes using boat-3
motor edge and past-win edge versus inside/outside opponents. Separate second
and third place role models are fitted only from prior-month reject races in
the same regime where boat 3 actually won. Missing regime/model => fail closed.
September outcomes are forbidden; v288 production stays fixed.
"""
...
OUT_JSON=Path('research_v289_3head_addon_wave8_motor_racer_role.json')
OUT_MD=Path('research_v289_3head_addon_wave8_motor_racer_role.md')
MIN_POSITIVE_RACES=5
VARIANTS=('motor_racer_role_product_target3','motor_racer_role_ev_target3','motor_racer_role_product_top10','motor_racer_role_ev_top10')
...
def regime(row):
    # All four inputs are pre-race-safe canonical v243 features. Requiring both
    # inside and outside comparisons avoids silently classifying partial data.
    mi=num(row,'f__c_b3_inside_motor'); mo=num(row,'f__c_b3_outside_motor')
    ri=num(row,'f__c_b3_inside_pastwin'); ro=num(row,'f__c_b3_outside_pastwin')
    if not all(np.isfinite(x) for x in (mi,mo,ri,ro)): return None
    motor=(mi+mo)/2.0; racer=(ri+ro)/2.0
    return ('M+' if motor>=0 else 'M-') + ('R+' if racer>=0 else 'R-')
...
def main():
            ...
            for _,cr in te.iterrows():
                ...
                for key,ts in ovs.items():
                    for mode in ('target3','top10'):
                        ...
                        name=f'motor_racer_{key}_{mode}'
    ...
    decision='SHADOW_CANDIDATE_WAVE8' if passers else 'NO_ADOPTION_WAVE8'
    bm=w1.metric(baseline); bm['max_drawdown_yen']=w1.max_drawdown(baseline)
    out={'research_version':'v289-addon-wave8-motor-racer-role','github_run_id':os.getenv('GITHUB_RUN_ID'),'source_max_date':q.date.max(),'waku10_validated_races':int(common),'rules':{'baseline_fixed':'v288 operational 94R','candidate_universe':'operational PRE S/A + original v242-buyable + final v288 NO_BET','test_months':'2026-02..2026-08','training':'prior-month reject races only, same motor/past-win regime, separate 2nd/3rd role models','regime':'sign of mean boat3 inside/outside motor edge x sign of mean boat3 inside/outside past-win edge; missing => fail closed','july_august':'NON-PRISTINE','september_outcomes_used':False,'not_threshold_relaxation':True,'min_positive_train_races':MIN_POSITIVE_RACES,'variants':list(VARIANTS)},'baseline':bm,'candidate_pool':w1.metric(candidates),'methods':rows,'passers':passers,'decision':decision,'audit':audit,'detail':detail}
    OUT_JSON.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    L=['# v289 3号艇 add-on auto research — Wave 8 motor/racer-conditioned role models','', '- v288 baseline **94R fixed / unchanged**','- target: final v288 NO_BET only','- pre-race regime: boat3 motor edge × past-win edge (4 regimes); separate 2着/3着 role models within each regime','- missing regime/model => fail closed','- July/August NON-PRISTINE; September outcomes not loaded','', '## Methods','| method | add R | hits | hit rate | ROI | profit | min month ROI | red months | max DD | overlap | combined R | combined ROI |','|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
    for x in rows:
    ...
    OUT_MD.write_text('\n'.join(L)+'\n',encoding='utf-8'); print('\n'.join(L)); print('V289_WAVE8_MOTOR_RACER_ROLE_OK')
