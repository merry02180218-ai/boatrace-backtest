...
def main():
 ...
 for mon in v234.MONTHS:
  ...
  for _,r in sel.iterrows():
   ...
   z={'month':mon,'date':r._date.strftime('%Y-%m-%d'),'race_code':code,'p3':float(r._p),'y3':int(r._y),'odds_available':0,'bet':0,'skip_below_top5':0,'invalid':0,'raw_top_n':np.nan,'raw_comp_odds':np.nan,'top_n':np.nan,'comp_odds':np.nan,'capped_at_10':0,'positive_tickets':np.nan,'trifecta_hit':0,'ret':np.nan,'profit':np.nan,'non_pristine':int(mon in ('2026-07','2026-08'))}
   if code in oi.index:
    z['odds_available']=1
 ...
 L=['# v242 variable TopN target composite odds 3.0: min Top5 / max Top10','', '- Frozen restored-Waku10 head selector + frozen V221 ordered-pair ranker; no tuning.', '- First find the unconstrained Top2..Top20 N closest to composite odds 3.00.', '- If unconstrained N < 5: NO BET.', '- If unconstrained N > 10: cap at Top10, even when Top10 composite odds is still >= 3.00.', '- Otherwise use Top5..Top10 as selected.', '- Exactly 10,000 yen per bet race with canonical inverse-odds Dutch / 100-yen Hamilton rounding.', '- Jul/Aug are NON-PRISTINE descriptive only; Dec-Jun are research/contaminated.', f'- Waku10 validation: {common} races, 0 mismatches.','', '|period|R|bets|skip<5|cap10|head hit|3-ren-tan hit/bet|avg TopN|avg comp odds|new ROI|','|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
 for label,mask in [('Dec-Jun',q.month<='2026-06'),('Jul-Aug',q.month>='2026-07'),('All',q.month.notna())]:
  g=q[mask];s=g[g.bet==1];R=len(g);B=len(s);skip=int(g.skip_below_top5.sum());cap=int(s.capped_at_10.sum());H=int(s.trifecta_hit.sum());head=100*g.y3.mean() if R else np.nan;roi=100*s.ret.sum()/(B*BANK) if B else np.nan
  L.append(f'|{label}|{R}|{B}|{skip}|{cap}|{head:.2f}%|{100*H/B if B else float("nan"):.2f}%|{s.top_n.mean():.2f}|{s.comp_odds.mean():.3f}|{roi:.2f}%|')
 L += ['', '## Monthly','|month|R|bets|skip<5|cap10|3-ren-tan hit/bet|avg TopN|avg comp odds|new ROI|status|','|---|---:|---:|---:|---:|---:|---:|---:|---:|---|']
 for mon,g in q.groupby('month',sort=True):
  s=g[g.bet==1];R=len(g);B=len(s);skip=int(g.skip_below_top5.sum());cap=int(s.capped_at_10.sum());H=int(s.trifecta_hit.sum());roi=100*s.ret.sum()/(B*BANK) if B else np.nan;st='NON-PRISTINE' if mon in ('2026-07','2026-08') else 'research/contaminated'
  L.append(f'|{mon}|{R}|{B}|{skip}|{cap}|{100*H/B if B else float("nan"):.2f}%|{s.top_n.mean():.2f}|{s.comp_odds.mean():.3f}|{roi:.2f}%|{st}|')
