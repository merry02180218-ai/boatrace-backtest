...
Committed reusable causal cache is mandatory for repeated opponent research:
...
Do not silently fall back to heavy full reconstruction if cache is missing. Fail fast, rebuild/refresh once only if upstream causal logic/provenance changes, cohort explicitly changes, or cache audit fails.
...
## v310 — causal p3/p4 head-risk
Best `HEADRISK_L2_3`:
- SECOND TOP2 72.07%
- exact3 133/345=38.55%
- boat4 38.10%, boat5 18.75%, boat6 0%
Conclusion: useful small auxiliary signal only.

## v311 — feature-family research
Development best `DROP_START`:
- SECOND TOP2 **72.41%**
- outer 4/5/6 TOP2 **26.44%**
- exact3 **136/345=39.42%**
- boat4 40.48%, boat5 18.75%, boat6 0%
- worst-month TOP2 60.00%
Development-only; unstable and outer 5/6 still unresolved.
...
- gate fired 11R, 9/11 actual outer
...
Interpretation: good boat4 recovery, no boat5/6 solution; do not endlessly micro-tune.

## v314 — role-split SECOND completed
Initial implementation failed because `FastSecond` assumed 5-candidate groups while specialists used inner 2 candidates / outer 3 candidates. Fixed with dynamic `FastListwise(group_n=len(boats))`, commit `29b944d825cb856eb3c0955e1a5f64e10932b755`.
Successful result commit: `d07beaa35c8bd4f62beb391e1cc877189cc34d21`.
Best role-split result was not a general improvement:
...
Interpretation: recovered some 5/6 but sacrificed inner 2/3 enough that overall stability worsened. Do not promote.

## v315 — pairwise SECOND completed
Latest v315 is **pairwise SECOND**, which supersedes the older early placeholder numbering for outer boat-specific research.
Files:
- `run_v315_1head_opponent_pairwise_second.py`
- `.github/workflows/research-20260913-v315-pairwise-second.yml`
- `summary_v315_1head_opponent_pairwise_second.md`
- result commit `2419b4e2887b47a6d64f9a4e2339ce1c15b760e1`

Uses candidate-vs-candidate logistic ranking with outer-winner weighting, v313 cache only, THIRD frozen v300 conditional L2=.1.
...
- SECOND TOP2 **72.76%** (+0.35pp only)
- outer **27.59%** (+1.15pp)
- exact3 **133/345=38.55%** (worse)
- worst month **60.00%**
- by actual SECOND TOP2: boat2 99.12%, boat3 83.15%, boat4 40.48%, boat5 18.75%, boat6 7.69%
- monthly TOP2: Feb 72.55%, Mar 60.00%, Apr 79.17%, May 76.24%, Jun 66.67%

Decision: pairwise gain is weak (<~+0.5pp) and exact3 worsened, so **do not micro-tune v315**. Move to a materially different formulation.

## v316 CURRENT ACTIVE — race-level multiclass SECOND with route/class balancing
Created after detecting that v315 completed and the chain had stopped.
Files:
...
Workflow commit `cbac8b159ab584d36ffb649b0751453aad5971ef`
...
- candidate feature blocks are concatenated in fixed boat order
- START family dropped to preserve v311 best base
- outer-route sample weights tested 1.0/1.5/2/3/4
- blend multiclass probability with DROP_START base at mixes .35/.50/.65/.80/1.0
- THIRD frozen at v300 conditional L2=.1 for clean SECOND attribution
- Feb-Jun development only; Jul/Aug NON-PRISTINE; September unread

Fresh automation/chat must inspect the latest v316 workflow status first. If v316 fails, inspect/fix causally safe implementation and restart. If it succeeds, record result here and continue automatically according to the decision rules below.
...
5. **v316 multiclass SECOND with route/class balancing — CURRENT.**
6. If v316 remains weak/unstable, perform error-driven PRE/prior feature engineering using only causally safe ST, motor, player/form, lane geometry, prior foot/turn, causal p3/p4.
7. When SECOND ceases to be dominant or further safe SECOND variants saturate, rebuild conditional THIRD separately.
8. If factorized SECOND×THIRD remains limiting, fit direct 20 ordered `(second,third)` pair model.
9. After probability/ranking models stabilize, optimize exactly-3-ticket policy.
10. Any suspicious large gain -> immediate leakage/causal audit.
...
- Outer capture improves while inner 2/3 collapses enough to reduce overall TOP2/exact3 => gated blend only, not wholesale replacement.
- When SECOND is no longer dominant error, move to THIRD/ordered-pair according to latest decomposition.
