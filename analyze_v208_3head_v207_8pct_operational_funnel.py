...
def main():
    ...
    final=prod[prod._code.isin(pre_codes)].copy()
    ...
    final['od3_settled']=final._code.isin(set(settled._code)).astype(int);final.to_csv(OUT,index=False,encoding='utf-8-sig')
    ...
    L=['# v208 — v207 8% PRE -> production FINAL operational replay','', '**Frozen rule: v207 8% = base+prior_ex, cut 0.280323. August is not retuned.**','', '## August untouched funnel','|stage|R|rate / note|','|---|---:|---|',f'|All races|{allR}|100%|',f'|v207 8% PRE watch|{preR}|{100*preR/allR:.1f}% of all; formal recall {100*preformal/formal:.1f}% ({preformal}/{formal})|',f'|Actual v165 FINAL pass (p3head>=30%)|{finalR}|{100*finalR/preR:.1f}% of PRE watch|',f'|od3-settled FINAL races|{settledR}|{100*settledR/finalR:.1f}% of FINAL pass|','', '## August untouched result after FINAL','- v166 tickets: frozen Top10 from production replay.','- Odds: BoatraceCSV od3 only; ¥10,000/race inverse-odds Dutch in ¥100 units.',f'- Top10 hit: **{hit}/{settledR} = {100*hit/settledR:.1f}%**',f'- Average composite odds: **{settled.composite_odds.mean():.3f}**',f'- Cost: **¥{cost:,.0f}**',f'- Return: **¥{ret:,.0f}**',f'- ROI: **{roi_pct:.1f}%**','', '## PICK UP inside FINAL',f'- FINAL PICK UP races: **{int(final.pickup.sum())}R**; od3 settled: **{len(pickup)}R**',f'- Settled PICK UP hit: **{int(pickup.top10_hit.sum())}/{len(pickup)} = {(100*pickup.top10_hit.mean() if len(pickup) else 0):.1f}%**',f'- Settled PICK UP ROI: **{(100*pr/pc if pc else 0):.1f}%**','', '## Audit','- PRE is determined before current-race exhibition. v165 FINAL uses current exhibition margins. v195 supplies the frozen production v166 Top10.',f'- reconstructed June={len(tr)}, August={len(te)}, unmatched={miss}; missing dates={missing_dates or "none"}.','- Production remains unchanged; this is SHADOW evaluation.']
    SUMMARY.write_text('\n'.join(L)+'\n',encoding='utf-8');print('\n'.join(L))
if __name__=='__main__':main()
# trigger: v208 workflow
