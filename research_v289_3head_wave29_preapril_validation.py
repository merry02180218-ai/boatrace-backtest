...
OUT=Path('analysis_v289_3head_wave29_preapril_validation.csv')
OUTJ=Path('research_v289_3head_wave29_preapril_validation.json')
OUTM=Path('research_v289_3head_wave29_preapril_validation.md')
...
def fit(X,y):
    m=make_pipeline(SimpleImputer(strategy='median'),StandardScaler(),LogisticRegression(max_iter=350,C=.35,solver='lbfgs'));m.fit(X,y);return m
def probs(m,X):
...
def main():
    ...
    # March is the only tuning month and is predicted from February only.
    tr=df['month']=='2026-02';te=df['month']=='2026-03';pp=probs(fit(X.loc[tr],y.loc[tr]),X.loc[te])
    for j,idx in enumerate(df.index[te]):P[pos[idx],:]=pp[j]
    # Apr-Jun untouched walk-forward validation.
    ...
    hold=df[df['month'].isin(['2026-04','2026-05','2026-06'])].copy();sel_h,hm=score(hold,t,k)
    ...
    out={'wave':'29-preapril-leakfree-validation','feature_count':63,'march_tuning_from_feb_only':chosen,'strict_pristine_holdout_apr_jun':hm,'shadow_non_pristine_jul_aug':sm,'legacy_overlap':0,'combined_baseline_plus_holdout':cmb,'decision':decision,'notes':['No 節D/current-meet result/ST fields.','Threshold and ticket count chosen only from March predictions trained on February.','Apr-Jun are fully untouched model-selection holdout.','Jul-Aug NON-PRISTINE shadow only; September forbidden; closing odds staking only.']}
    OUTJ.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    lines=['# Wave29 pre-April leak-free validation','',f"- March tune (train Feb only): **p3>={t:.2f}, top{k}; {chosen['races']}R / ROI {chosen['roi_pct']:.3f}%**",f"- Apr-Jun untouched holdout: **{hm['races']}R / {hm['hits']} hits / ROI {hm['roi_pct']:.3f}% / profit {hm['profit_yen']:+,} yen**",f"- min month: **{hm['min_month_roi_pct']:.3f}%** / red months **{hm['red_months']}** / max DD **{hm['max_drawdown_yen']:,.0f} yen**",f"- Jul-Aug NON-PRISTINE shadow: **{sm['races']}R / {sm['hits']} hits / ROI {sm['roi_pct']:.3f}% / profit {sm['profit_yen']:+,} yen**",f"- baseline + holdout: **{cmb['races']}R / ROI {cmb['roi_pct']:.3f}% / profit {cmb['profit_yen']:+,} yen**",'- legacy overlap: **0**',f'- decision: **{decision}**','','## Pristine holdout monthly']
