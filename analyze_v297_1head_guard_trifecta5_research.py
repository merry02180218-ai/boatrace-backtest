...
def settle_full_after_freeze(d):
    """Attach winner + exact top-3 only after the prediction-time feature frame is frozen."""
    ...
    if cov<.985:raise RuntimeError(f'direct exact-order settlement coverage too low {cov:.4f}')
    return z,cov
...
def combo_predict(fs,tr,te):
    ...
    if len(classes)!=20:raise RuntimeError(f'exact-order model has {len(classes)} classes, expected 20')
    out=[]
    for row in pro:
        ix=np.argsort(-row)
...
def head_fold(tr,te,core_fs,guard_fs):
    ...
    oop_h=v293.cal(ch,oo_h.p);ph=v293.cal(ch,v293.fitpred('hgb',core_fs,tr,te))
    oo_g=v293.oof('lr',guard_fs,tr);cg=v293.calfit(oo_g)
    oop_g=v293.cal(cg,oo_g.p);pg=v293.cal(cg,v293.fitpred('lr',guard_fs,tr,te))
    hc={q:float(np.quantile(oop_h,q)) for q in HEAD_Q};gc={q:float(np.quantile(oop_g,q)) for q in GUARD_Q}
...
def run(d,fams):
    ...
    for tm in TEST_MONTHS:
        ...
        z['top5']=[';'.join(t) for t,_ in cp];z['top5_mass']=[m for _,m in cp]
        actual=z.actual_combo.to_numpy()
        for k in K_LIST:z[f'hit{k}']=[int(bool(a) and a in tickets.split(';')[:k]) for a,tickets in zip(actual,z.top5)]
        pred.append(z)
        folds.append({'month':tm,'core_features':len(core),'guard_features':len(guard),
                      'combo_features':len(cfs),'train':len(tr),'test':len(te),
                      'head_oof':noh,'guard_oof':nog,'combo_train_headwins':ncombo,'combo_classes':nclasses})
...
def evaluate(p):
    ...
    for name,hq,gq in selector_specs():
        for conf in CONF_CUTS:
            ...
            mg=pd.DataFrame(monthly[start:]);R=int(mg.R.sum());H=int(mg.heads.sum());T=int(mg.hit5.sum());v=mg[mg.R>0]
            ...
            r['head_wilson_lo'],r['head_wilson_hi']=wilson(H,R);r['tri5_wilson_lo'],r['tri5_wilson_hi']=wilson(T,R)
            pooled.append(r)
...
def k_metrics(p,pool):
    ...
    name=best['rule'];conf=float(best.conf_cut);_,hq,gq=next(x for x in selector_specs() if x[0]==name)
    s=p[mask_rule(p,hq,gq,conf)].copy();out=[]
    for k in K_LIST:
        H=int(s[f'hit{k}'].sum());R=len(s);lo,hi=wilson(H,R)
        out.append({'rule':name,'conf_cut':conf,'points':k,'R':R,'hits':H,'hit_rate':H/R if R else np.nan,'wilson_lo':lo,'wilson_hi':hi})
...
def make_summary(cov,folds,monthly,pool,km):
    ...
    L=['# v297 clean 1HEAD loss-guard + trifecta <=5 research','',
       ...
       '- Exact-order settlement is fetched directly from historical realtime result CSV only after race-card/history features are frozen.',
       ...
       f'- Direct exact-order settlement coverage: **{100*cov:.2f}%**.','',
       '## Fold audit','|month|train|test|head features|guard features|combo features|combo train 1-head|classes|',
       '|---|---:|---:|---:|---:|---:|---:|---:|']
    for _,r in folds.iterrows():L.append(f"|{r.month}|{int(r.train)}|{int(r.test)}|{int(r.core_features)}|{int(r.guard_features)}|{int(r.combo_features)}|{int(r.combo_train_headwins)}|{int(r.combo_classes)}|")
    ...
    if headok.empty:L.append('|None|-|-|-|-|-|-|-|-|-|')
        ...
        for _,r in headok.head(30).iterrows():L.append(f"|{r.rule}|{r.conf_cut:.2f}|{int(r.R)}|{pct(r.head_rate)}|{pct(r.head_wilson_lo)}|{pct(r.trifecta5_rate)}|{pct(r.tri5_wilson_lo)}|{pct(r.worst_head_month)}|{pct(r.worst_tri5_month)}|{int(r.min_month_R)}|")
    ...
    if target.empty:
        ...
        for _,r in target.sort_values(['tri5_wilson_lo','head_wilson_lo','R'],ascending=False).head(10).iterrows():L.append(f"  - {r.rule}, conf>={r.conf_cut:.2f}: R={int(r.R)}, head={pct(r.head_rate)}, 5pt={pct(r.trifecta5_rate)}")
...
def main():
    d,a=v294.freeze_true_pre();d=v294.add_prior_history(d);d=v294.add_rel(d);fams=v296.clean_manifest(d)
    for n,fs in fams.items():
        bad=[c for c in fs if 'meet_' in c]
        if bad:raise RuntimeError(f'forbidden meeting feature in {n}: {bad[:4]}')
    print('features frozen; direct exact-order settlement now',flush=True)
    d,cov=settle_full_after_freeze(d);d=d[(d.valid_result==1)&(d.combo_valid==1)].copy()
    ...
    SUMMARY.write_text(make_summary(cov,folds,monthly,pool,km),encoding='utf-8');print(SUMMARY.read_text(),flush=True)
