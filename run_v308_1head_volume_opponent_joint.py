...
P=ROOT/'analysis_v308_1head_volume_opponent_joint'
S=ROOT/'summary_v308_1head_volume_opponent_joint.md'
TM=list(v298.TEST_MONTHS)

def main():
 v300.setup_v298()
 d,_=v298.v294.freeze_true_pre(); d=v298.v294.add_prior_history(d); d=v298.v294.add_rel(d); d,th=v298.add_threat(d)
 ...
 # Build v300 opponent long tables once. setup_v298() patches v298.second_long/conditional_long
 # to the all-race inference-safe implementations used by the successful v300 research.
 ...
 for tm in TM:
  ...
  z=te[['date','month','race_code','venue','race','head_hit','actual_combo']].copy(); z['test_month']=tm
  ...
  ph,_,_,_,_,_=v298.head_fold(tr,te,list(dict.fromkeys(bc+ex)),bg); z['p_head']=ph

  # v300 best opponent model: augmented SECOND L2=3 / THIRD L2=.1, TOP2xTOP2 alpha=.60.
  p2,_=v300.p2_predict(sl,tm,3.0,True)
  pc,_=v300.pc_predict(cl,tm,.1,True)
  tickets=[]; hits=[]
  for _,r in z.iterrows():
   code=str(r.race_code).zfill(12)
   cand=v300.ticket3(p2[code],pc[code]).split(';')
   tickets.append('|'.join(cand))
   hits.append(int(str(r.actual_combo) in cand))
  z['tickets']=tickets; z['exact3']=hits
 ...
 hcut=float(p.p_head.quantile(float(best.q)))
 sel=p[(p.p_head>=hcut)&(p.opp_mass>=float(best.conf))].copy()
 gm=sel.groupby('test_month').agg(R=('head_hit','size'),head_hits=('head_hit','sum'),exact3_hits=('exact3','sum')).reset_index()
 gm['head_rate']=gm.head_hits/gm.R; gm['exact3_rate']=gm.exact3_hits/gm.R
