#!/usr/bin/env python3
"""Sequential six-boat tracker with perspective-trend compensation.

Calibration/research tooling only. Result-blind: it reads exhibition video and seed
ROIs, never race results. Compared with v1, this version reseeds sparse features on
every frame to survive large camera pan. At each horizon it removes the first-order
screen-motion trend versus lane/image y, which is the dominant perspective effect in
BOATCAST start-exhibition side shots.

This is intentionally fail-closed. SES is emitted only when all six boats have enough
tracking support and lane order remains intact.
"""
...
def local_motion(prev_g,cur_g,cx,cy,roi_w=78,roi_h=48):
    mask=np.zeros_like(prev_g)
    x1=max(0,int(round(cx-roi_w*0.38))); x2=min(prev_g.shape[1],int(round(cx+roi_w*0.62)))
...
def robust_linear_residual(y,values):
    ...
    pred=X@beta
    return v-pred, {'intercept':float(beta[0]),'slope_per_y_px':float(beta[1])}
...
def main():
    ...
    ap.add_argument('--out',type=Path,default=Path('exhibition_tracking_v2.json'))
    args=ap.parse_args()
    ...
    for fi in range(1,nmax+1):
        ...
        centers += D
        if not np.all(np.diff(centers[:,1])>4):
            raise SystemExit(f'FAIL_CLOSED: lane-order violation at frame {fi}')
        prev_g=cur_g
    ...
    if not accepted:
        ...
        payload['boats']={str(i+1):{'relative_forward_1_5px':round(float(final_res[i]),3),
                                    'stretch_rank':int(ranks[i]),'ses_v2':int(ses[i])} for i in range(6)}
        payload['ses_scale_px']=round(float(scale),3)
        payload['warning']='EXPERIMENTAL calibration score; perspective correction is first-order and not yet validated across venues.'
