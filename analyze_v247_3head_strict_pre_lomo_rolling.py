...
# Same canonical final 182R target used by v246.
def final_mask(q):
...
# Conservative strict-PRE whitelist: only features known before the current race's exhibition.
# Explicitly reject current exhibition/ST/lap/turn/straight/original/tilt and post-selection fields.
...
def fit_rank(train, test, cols, ycol='_target', maxfeat=12):
    # Learn only from training months. Effect direction and magnitude are frozen before test scoring.
    feats=[]
    ...
    for _,eff,c in feats:
        ...
        if not np.isfinite(sd) or sd<=1e-12: continue
        z=(te.fillna(med)-med)/sd
        score += np.sign(eff)*z.clip(-4,4)
        used.append(c)
    return score,used
...
def main():
    ...
    rec=[]
    # LOMO: train on every other month, test only held-out month.
    for m in months:
        tr=q[q.month!=m]; te=q[q.month==m].copy()
        score,used=fit_rank(tr,te,cols)
        te['_score']=score
        # Candidate fractions mirror v246 operational volumes: 67%, 80%, 94% of monthly universe.
        for frac in (.67,.80,.94):
            k=max(1,int(np.ceil(len(te)*frac)))
            g=te.nlargest(k,'_score'); n,t,r=metrics(g)
            total=int(te._target.sum())
    ...
    # Rolling: train only on prior months. Require >=2 prior months to reduce tiny training instability.
    for i,m in enumerate(months):
        if i<2: continue
        tr=q[q.month.isin(months[:i])]; te=q[q.month==m].copy()
        score,used=fit_rank(tr,te,cols); te['_score']=score
        for frac in (.67,.80,.94):
            k=max(1,int(np.ceil(len(te)*frac)))
            g=te.nlargest(k,'_score'); n,t,r=metrics(g); total=int(te._target.sum())
    ...
    L=['# v247 strict PRE LOMO / rolling validation','',
       '- Target is canonical v243 182R policy (70 hits, historical ROI 119.73% in the source period).',
    ...
    for mode in ['LOMO','ROLLING']:
      for frac in (.67,.80,.94):
        z=out[(out['mode']==mode)&(out.candidate_frac==frac)]
        total=int(z.target_total.sum()); cap=int(z.captured.sum())
